...
**These instruments (plus `PERRY_GC_CENSUS`, `PERRY_GC_FROMSPACE_SCAN*`, `PERRY_ALLOC_SITE_SAMPLE`, `PERRY_GC_VERIFY_MARK`, `PERRY_GC_VERIFY_CLASSIFIER`) are compiled in only with perry-runtime's `gc-instruments` feature**, which auto-optimize adds when one of the knobs or `PERRY_GC_INSTRUMENTS=1` is set *at compile time*; a binary built without it aborts at startup if a knob is set, rather than silently running nothing.
