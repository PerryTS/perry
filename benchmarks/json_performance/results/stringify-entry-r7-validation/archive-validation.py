...
dest=bench/'results/stringify-entry-r7-validation'
...
for source in sorted(work.rglob('*')):
    ...
    compressed=source.suffix in {'.log','.stdout','.stderr','.ll','.s'} or source.name=='callback-only.ts'
...
manifest=dict(measured_source_commit=json.loads((work/'provenance.json').read_text())['source_commit'],
              note='R7 outlined stringify entry. Exact production build and linked worker hashes retained. Native fixture findings remain unsuppressed and are fingerprint-compared against actual frozen baselines; callback-only native control is clean. Both parsed-empty-array gaps reproduce on frozen main and R5. Script lint public benchmark freshness fails; compile tier and full matrices not run.',files=entries)
