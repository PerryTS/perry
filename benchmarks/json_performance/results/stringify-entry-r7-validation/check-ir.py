...
env |= {'PERRY_RUNTIME_DIR':str(work/'frozen-build'), 'PERRY_WORKSPACE_ROOT':str(root),
...
sources = [root/'test-files/test_json_invariant_field_loop.ts',
           root/'test-files/test_json_scalar_projection.ts',
           root/'test-files/test_json_stringify_entry.ts',
...
for mode, rs4gc in ([('shadow','0')] if '--shadow-only' in sys.argv else [('native','1'), ('shadow','0')]):
    ...
    for source in sources:
        ...
        scratch.mkdir()
        cmd = [str(work/'frozen-build/perry'), 'compile', str(source), '--no-auto-optimize',
        ...
        for index, ll in enumerate(files):
            ...
            invariants += len(re.findall(r"\bcall\s+double\s+@js_array_index_own_number\(", original))
            ...
            if mode == 'native':
                ...
                for line in dest.read_text().splitlines():
                    assert not ('gc.statepoint' in line and ('@js_json_lazy_index_scalar' in line or '@js_array_index_own_number' in line)), line
            else:
                dest.write_bytes(ll.read_bytes())
        if source.stem != 'test_json_stringify_entry': assert calls > 0, (source, mode, calls, 'scalar probe not emitted')
        if source.stem in ['access-worker', 'test_json_invariant_field_loop']:
            assert invariants > 0, (source, mode, 'invariant loop never emitted')
        counts.append(dict(source=str(source.relative_to(root)),lowering=mode,probe_calls=calls,invariant_probe_calls=invariants))
    ...
    variants = [['--statepoints','--max-stale','0','--min-files','5','--min-statepoints','1','--min-live-bundles','1','--min-relocates','1']] if mode=='native' else [[],['--unrooted-allocas']]
    ...
    if mode == 'native':
        workers = [str(p) for p in output.glob('*.ll') if not p.name.startswith('test_json_')]
        with (output/'workers-check.log').open('wb') as log:
            result = subprocess.run(checker+['--statepoints','--max-stale','0','--min-files','2','--min-statepoints','1','--min-live-bundles','1','--min-relocates','1']+workers,
