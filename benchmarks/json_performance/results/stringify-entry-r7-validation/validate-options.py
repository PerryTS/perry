...
references=[('main',w/'main-options'),('r5',w/'r5-options')]
if '--candidate' in sys.argv:references=[('candidate',w/'candidate-options')]
cases=[('small_record',x) for x in ['zero','pretty','keys','callback']]+[('records_array_16k','pretty')]
records=[]
...
(w/('candidate-options-validation.json' if '--candidate' in sys.argv else 'reference-options-validation.json')).write_text(json.dumps(records,indent=2)+'\n')
