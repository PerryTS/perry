...
for path,digest in provenance['files'].items():assert hashlib.sha256((root/path).read_bytes()).hexdigest()==digest,path
assert (work/'validation.json').exists()
assert all(x['same'] for x in json.loads((work/'root-fingerprint-comparison.json').read_text()))
assert all(x['matches_node'] and x['exit_code']==0 for x in json.loads((work/'existing-validation.json').read_text()))
assert all(x['matches_node'] for x in json.loads((work/'candidate-options-validation.json').read_text()))
assert json.loads((work/'candidate-callback-proof.json').read_text())['moved_objects']>0
assert all(x['matches_main_and_r5'] for x in json.loads((work/'candidate-inherited-gaps.json').read_text()))
assert next(x for x in json.loads((work/'candidate-extra-ir.json').read_text()) if x['subject']=='candidate-callback')['exit_code']==0
...
token='json-r7-stage-'+str(os.getpid())
def ssh(code):
...
try:
    ...
    files+=['.work/'+work.name+'/'+n for n in ['worker','access-worker','rotating-worker',
            ...
            'run-access.py','build-provenance.json','main-options','r5-options','candidate-options','run-options.py','options-worker.ts','options-worker.js','candidate-options-provenance.json','reference-options-provenance.json','candidate-options-validation.json','reference-options-validation.json']]
