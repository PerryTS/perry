...
if '--write' in sys.argv:
 ...
 paths=[bench/'ISOLATED_JSON_SCAN_CHUNKS_R36.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 assert (timed,verify,calibration)==(588,140,84)
 m=json.loads((w/'provenance.json').read_text())
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=timed,verification_records=verify,calibration_trials=calibration,qualification='Not promoted: fresh ASCII/escaped/Unicode and repeated escaped regressions; new valid-Korean fresh regressions up to 280.61%. Two rotating windows only; no current-source stringify-option or full-matrix performance window. Frozen R26 reference at 0.5.1531; not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
