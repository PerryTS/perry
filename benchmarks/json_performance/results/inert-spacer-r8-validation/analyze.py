...
phases = ['access', 'options'] if recheck else ['access', 'focus', 'options']
for kind in phases:
    ...
    total += len(timings)
assert total == (704 if recheck else 952), total
(work / ('recheck-analysis.json' if recheck else 'analysis.json')).write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
