...
dest=bench/'results/scalar-projection-r2-validation'
dest.mkdir(exist_ok=False)
entries=[]
allowed={'.py','.json','.ts','.log','.stdout','.stderr','.ll','.md','.rs'}
for source in sorted(work.rglob('*')):
    ...
    compressed=source.suffix in {'.log','.stdout','.stderr','.ll'}
...
manifest=dict(measured_source_commit=json.loads((work/'provenance.json').read_text())['source_commit'],
              note='Measured binaries are frozen at this commit. The second runtime unit pass includes a test-only canonical string-accessor correction; product source is unchanged.',files=entries)
