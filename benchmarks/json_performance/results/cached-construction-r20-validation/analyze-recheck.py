...
phases = ['focus']
for kind in phases:
    d = bench / 'results' / ('quiet-' + work.name + '-recheck-' + kind)
    ...
    assert len(verifies) == 5 and len(timings) == 44 and len(summary) == 4
    ...
    declarations = {(c['fixture'], c['operation']): c for c in json.loads((work / 'recheck-cases.json').read_text())['cases']}
    ...
    assert len(by_case) == {'access': 12, 'focus': 1, 'options': 4 if recheck else 7, 'full': 5 if full_recheck else 50}[kind]
...
(work / 'recheck-analysis.json').write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
