...
bench = w.parents[1]
d = bench / 'results' / ('quiet-' + w.name + '-recheck-rotating')
...
assert len(timing) == 88 and len(calibration) == 8 and len(verify) == 16
...
assert len(output) == 2
counts = json.loads((w / 'recheck-counts.json').read_text())
initial = json.loads((w / 'rotating-analysis.json').read_text())['cases']
for case in output:
    first = next(c for c in initial if c['fixture'] == case['fixture'] and c['mode'] == case['mode'])
    assert case['engines']['perry']['iterations'] == counts['counts'][case['mode']] == 4 * first['engines']['perry']['iterations']
staged = json.loads((w / 'remote-recheck-stage-hashes.json').read_text())
assert len(staged) == 104 and all(sha(bench / p) == digest for p, digest in staged.items())
(w / 'rotating-recheck-analysis.json').write_text(json.dumps(dict(trials=88, calibration_trials=8,
    verification_trials=16, window=window, cases=output), indent=2) + '\n')
print('Verified all 88 timing, 8 calibration, 16 full-output checks, hashes and quiet window.')
