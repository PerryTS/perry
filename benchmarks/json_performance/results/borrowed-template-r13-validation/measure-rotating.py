...
(w / 'remote-rotating-command.json').write_text(json.dumps(cmd, indent=2) + '\n')
with (w / 'remote-rotating.log').open('wb') as log:
    result = subprocess.run(['ssh', 'perry@perry-macos.local',
                             'cd ' + shlex.quote(remote) + ' && ' + shlex.join(cmd)],
                            stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-rotating.log').read_text(), flush=True)
