...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-access-profiles.py','profile-cases.json','r32-access-worker','workers-provenance.json','build-provenance.json','source.patch','access-worker.ts','access-worker.js']]+['.work/fixtures/'+f+'.json' for f in ['records_array_1m']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-access-r32-profiles-'+str(os.getpid())
assert json.loads((w/'build-provenance.json').read_text())['source_commit']=='c695f6509ed85999894a2f34418f057f32bcb363'
proof=next(r for r in json.loads((w/'workers-provenance.json').read_text()) if r['worker']=='access-worker')
assert expected[prefix+'r32-access-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-access-worker'))
...
slug='quiet-'+w.name;cmd=['python3','with_lock.py','--','python3',prefix+'run-access-profiles.py','--results-dir','results/'+slug]
...
for n in ['run-access-profiles.py','profile-cases.json','workers-provenance.json','build-provenance.json','source.patch','access-worker.ts','access-worker.js']:shutil.copy2(root/PREFIX/n,d/n)
