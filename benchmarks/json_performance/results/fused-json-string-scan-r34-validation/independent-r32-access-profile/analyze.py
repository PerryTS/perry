...
for row in json.loads((d/'profiles.json').read_text()):
 c=row['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode'];assert row['worker_exit']==row['sampler_exit']==0 and row['stop_reason'] is None and row['diagnostic_only'];assert row['worker_sha256']==sha((w/'r32-access-worker').read_bytes())
 assert subprocess.check_output(['/opt/homebrew/bin/node','--version'],text=True).strip()=='v26.5.1'
 cmd=['/opt/homebrew/bin/node',str(w/'access-worker.js'),str(bench/'.work/fixtures'/(c['fixture']+'.json')),c['operation'],str(c['iterations']),str(c['warmup']),'verify']
 ...
 phases={'workload':phase(work),'index_get':phase(lambda n:'js_packed_arraylike_index_get' in n),'dynamic_remainder':phase(lambda n:'js_dynamic_mod' in n),'libm_fmod':phase(lambda n:'fmod' in n),'property_lookup':phase(lambda n:any(x in n for x in ['js_get_property','js_object_get_field','js_nanbox_get_field'])),'root_scope':phase(lambda n:'RuntimeHandleScope' in n),'utf8_validation':phase(lambda n:'utf8' in n.lower()),'memory_copy':phase(lambda n:'memcpy' in n or 'memmove' in n),'allocation':phase(lambda n:any(x in n for x in ['gc_alloc','gc_bump','string_storage_alloc','object_alloc'])),'collection':phase(lambda n:any(x in n for x in ['gc_safepoint','gc_collect','gc_budgeted']))}
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='c695f6509ed85999894a2f34418f057f32bcb363',diagnostic_only=True,note='Two R32 frozen-worker access sampling diagnostics, with Node executing the complete identical loop count and warmup; checksum and KEEP verified. Access worker does not emit full JSON output. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
