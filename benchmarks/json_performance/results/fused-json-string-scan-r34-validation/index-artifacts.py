...
if '--write' in sys.argv:
 ...
 paths=[bench/'FUSED_JSON_STRING_SCAN_R34.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=timed,verification_records=verify,calibration_trials=calibration,qualification='Not promoted: separated fresh small/record-array/ASCII/Unicode parse regressions and repeated Unicode regression. Stringify option gains do not qualify the combined source. Controlled R26 comparison at0.5.1531; not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
