...
source = root / 'test-files/test_json_source_token_length.ts'
...
(w / 'token-node.stdout').write_bytes(node.stdout)
(w / 'token-node.stderr').write_bytes(node.stderr)
assert node.returncode == 0, node.stderr
binary = w / (arm + '-token')
cmd = [str(build / 'perry'), 'compile', str(source), '--no-auto-optimize', '--no-cache', '-o', str(binary)]
with (w / (arm + '-token-compile.log')).open('wb') as log:
...
for parser, settings in [('auto', {}), ('tape', {'PERRY_JSON_TAPE': '1'}), ('direct', {'PERRY_JSON_TAPE': '0'})]:
    for mode, knobs in [('normal', {}), ('scheduled', stress), ('fullgc', {'PERRY_GEN_GC': '0'})]:
        ...
        rows.append(row)
        (w / (arm + '-token-validation.json')).write_text(json.dumps({'command': cmd, 'rows': rows, 'files': {str(p.relative_to(root)): hashlib.sha256(p.read_bytes()).hexdigest() for p in [source, binary, build / 'perry', build / 'libperry_runtime.a', build / 'libperry_stdlib.a']}}, indent=2) + '\n')
