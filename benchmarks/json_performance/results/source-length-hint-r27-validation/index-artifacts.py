...
if '--write' in sys.argv:
 ...
 paths=[bench/'SOURCE_TOKEN_LENGTH_R27.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=timed,verification_records=verify,calibration_trials=calibration,qualification='Unpromoted R27 screen at 0.5.1531 against frozen R26: large-token gains and reproduced small-record regression. Broader performance qualification deferred. All validation limitations disclosed.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
