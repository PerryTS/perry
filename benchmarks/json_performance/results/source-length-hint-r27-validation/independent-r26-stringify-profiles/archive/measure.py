...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-stringify-profiles.py','profile-cases.json','r26-options-worker','workers-provenance.json','build-provenance.json','source.patch','options-worker.ts','options-worker.js']]+['.work/fixtures/'+f+'.json' for f in ['small_record','records_array_16k']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-stringify-r26-profiles-'+str(os.getpid())
...
ssh(code)
# Archive locally under ignored WORK while the independent R27 build requires a clean source tree.
