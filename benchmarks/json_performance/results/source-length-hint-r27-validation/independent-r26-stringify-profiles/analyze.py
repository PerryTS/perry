...
for row in json.loads((d/'profiles.json').read_text()):
 c=row['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode'];assert row['worker_exit']==row['sampler_exit']==0 and row['stop_reason'] is None and row['diagnostic_only'];assert row['worker_sha256']==sha((w/'r26-options-worker').read_bytes())
 assert subprocess.check_output(['/opt/homebrew/bin/node','--version'],text=True).strip()=='v26.5.1'
 cmd=['/opt/homebrew/bin/node',str(w/'options-worker.js'),str(bench/'.work/fixtures'/(c['fixture']+'.json')),c['operation'],'8','0','verify']
 ...
 assert result[5]==expected[5]/8*(c['iterations']+c['warmup']) and result[6]==0
 ...
 def work(n):return 'perry_fn_options_worker_ts__run' in n
 def phase(pred):
 ...
 rows.append(dict(case=c,workload_samples=total,phases=phases,top_self_sample_nodes=leaves,profile_sha256=sha(text.encode()),oracle_command=cmd,oracle_output_sha256=sha(oracle.stdout),complete_output_matches_node=True,diagnostic_only=True))
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',diagnostic_only=True,note='Three R26 frozen-worker stringify sampling diagnostics, with complete Node output and checksum verification. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
