...
for c in cases:
 label=c['fixture']+'-'+c['operation']+'-'+c['mode'];env=clean;worker=w/(c['mode']+'-worker')
 ...
 with (d/(label+'.stdout')).open('wb') as out,(d/(label+'.stderr')).open('wb') as err:
  proc=subprocess.Popen(cmd,env=env,stdout=out,stderr=err,start_new_session=True);started=time.monotonic();sample_cmd=['/usr/bin/sample',str(proc.pid),'1','1','-mayDie','-file',str(d/(label+'.sample.txt'))]
