...
phases = ['focus']
for kind in phases:
    d = bench / 'results' / ('quiet-' + work.name + '-r22-control-focus')
    ...
    assert raw == (work / 'r22-control-source.patch').read_bytes()
    ...
    if kind == 'access':
        ...
        for engine, arm in [('perry', 'r22-shared'), ('baseline', 'main')]:
    ...
    staged = json.loads((work / 'r22-control-stage-hashes.json').read_text())
...
(work / 'r22-control-analysis.json').write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
