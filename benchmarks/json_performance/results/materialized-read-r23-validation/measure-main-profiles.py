...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-access-profiles.py','profile-cases.json','main-access-worker','main-workers-provenance.json','main-build-provenance.json','reference-artifacts.json']]+['.work/fixtures/'+f+'.json' for f in ['records_array_16k','records_array_1m']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-r23-main-profiles-'+str(os.getpid())
...
(w/'main-profile-stage-hashes.json').write_text(json.dumps(expected,indent=2)+'\n')
slug='quiet-'+w.name+'-main-profiles';cmd=['python3','with_lock.py','--','python3',prefix+'run-access-profiles.py','--results-dir','results/'+slug]
(w/'main-profile-command.json').write_text(json.dumps(cmd,indent=2)+'\n')
with (w/'main-profile-remote.log').open('wb') as log:r=subprocess.run(['ssh',host,'cd '+shlex.quote(remote)+' && '+shlex.join(cmd)],stdout=log,stderr=subprocess.STDOUT)
...
for n in ['run-access-profiles.py','profile-cases.json','main-workers-provenance.json','main-build-provenance.json','reference-artifacts.json']:shutil.copy2(root/PREFIX/n,d/n)
...
for p in [Path(__file__),w/'main-profile-command.json',w/'main-profile-stage-hashes.json',w/'main-profile-remote.log']:shutil.copy2(p,d/p.name)
(d/'controller-exit.json').write_text(json.dumps({'exit_code':r.returncode},indent=2)+'\n')
print('ARCHIVED',slug,flush=True);print((w/'main-profile-remote.log').read_text(),flush=True)
