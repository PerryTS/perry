...
for c in cases:
 label=c['fixture']+'-'+c['operation']+'-'+c['mode']+'-'+c['arm'];env=clean;worker=w/(c['arm']+'-rotating-worker')
 cmd=[str(worker),str(bench/'.work/rotating'/c['fixture']),c['operation'],str(c['iterations']),str(c['warmup']),'verify',c['mode']]
