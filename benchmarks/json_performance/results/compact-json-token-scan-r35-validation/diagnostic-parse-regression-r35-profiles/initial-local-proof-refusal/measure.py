...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-parse-regression-profile.py','profile-cases.json','r35-rotating-worker','r26-rotating-worker','workers-provenance.json','main-workers-provenance.json','build-provenance.json','main-build-provenance.json','source.patch','rotating-worker.ts','rotating-worker.js']]+['.work/rotating/'+f+'.'+str(i)+'.json' for f in ['records_array_1m','escaped_1m'] for i in range(8)]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-parse-regression-r35-profile-'+str(os.getpid())
assert json.loads((w/'build-provenance.json').read_text())['source_commit']=='891753f2cff604c87133ac2a4435af9a2286aa22'
proof=next(r for r in json.loads((w/'workers-provenance.json').read_text()) if r['worker']=='rotating-worker')
assert expected[prefix+'r35-rotating-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-rotating-worker'))
reference=next(r for r in json.loads((w/'main-workers-provenance.json').read_text()) if r['worker']=='rotating-worker')
assert expected[prefix+'r26-rotating-worker']==next(h for p,h in reference['files'].items() if p.endswith('/main-rotating-worker'))
...
for n in ['run-parse-regression-profile.py','profile-cases.json','workers-provenance.json','main-workers-provenance.json','build-provenance.json','main-build-provenance.json','source.patch','rotating-worker.ts','rotating-worker.js']:shutil.copy2(root/PREFIX/n,d/n)
