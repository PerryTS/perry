...
for row in json.loads((d/'profiles.json').read_text()):
 c=row['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode']+'-'+c['arm'];assert row['worker_exit']==row['sampler_exit']==0 and row['stop_reason'] is None and row['diagnostic_only'];assert row['worker_sha256']==sha((w/(c['arm']+'-rotating-worker')).read_bytes())
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='891753f2cff604c87133ac2a4435af9a2286aa22',reference_source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',diagnostic_only=True,note='Four frozen R26/R35 record-array and escaped-string rotating-parse diagnostics; pinned Node executes the complete identical loop and warmup, and checksum plus every VERIFY/LAST/KEEP byte match. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
