...
bench = w.parents[1]
d = bench / 'results' / ('quiet-' + w.name + '-korean-rotating')
...
stage = json.loads((w / 'korean-stage-hashes.json').read_text())
for f in host['fixtures']:
    for i, digest in enumerate(f['sha256']):
        path = '.work/' + w.name + '/korean-fixtures/' + f['fixture'] + '.' + str(i) + '.json'
...
assert len(timing) == 168 and len(calibration) == 24 and len(verify) == 40
...
assert len(output) == 6
(w / 'korean-rotating-analysis.json').write_text(json.dumps(dict(trials=168, calibration_trials=24,
    verification_trials=40, window=window, cases=output), indent=2) + '\n')
print('Verified all 168 timing, 24 calibration, 40 full-output checks, hashes and quiet window.')
