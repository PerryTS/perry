...
w=Path(__file__).resolve().parent;root=w.parents[3];old=w.with_name('fused-json-string-scan-r34');sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()
assert head=='891753f2cff604c87133ac2a4435af9a2286aa22'
...
def copy(p,d,adapt=False):
 ...
 if adapt:
  d.write_text(p.read_text().replace('fused-json-string-scan-r34','compact-json-token-scan-r35').replace('FUSED_JSON_STRING_SCAN_R34','COMPACT_JSON_TOKEN_SCAN_R35').replace('R34','R35').replace("'r34'","'r35'").replace('json-r34-','json-r35-'))
...
(w/'base.json').write_text(json.dumps(dict(base_commit='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d',reference_name='R26',branch_parent_source=head,previous_candidate_source='9b1bf09c92f7f1bc9f918d1b8c3adb554c3b6792'),indent=2)+'\n')
(w/'main-reuse-provenance.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',reference_evidence_commit='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d',copy_via_r34_evidence='4fbe1f3d046ff4a26ae4e4def0bba199f2c96509',note='Exact frozen R26 reference copied through R34. Reuse81original plus20emitter plus4primitive reference executions and28IR only with source/fixture equivalence. Primitive native finding is preserved unsuppressed; shadowchecks pass. All candidate executions will be fresh. No current-main or R34-candidate reference.',files=records),indent=2)+'\n')
(w/'measurement-plan.md').write_text('R35 first validates all105candidate executions,28IR andsixworkerobjects, preserving knownrootfindings. Measure rotating15 first, thenoptions7, largepretty/callback6, full50 andchanging2ifqualified. Ifpositive onhistoricalrows, add bounded Korean-string performance controls because ED falsepositive handling changed. NoGCpolicy orcacheadmission changes.\n')
