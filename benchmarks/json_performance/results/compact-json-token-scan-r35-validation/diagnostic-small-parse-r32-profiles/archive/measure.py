...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-small-parse-profile.py','profile-cases.json','r32-rotating-worker','workers-provenance.json','build-provenance.json','source.patch','rotating-worker.ts','rotating-worker.js']]+['.work/rotating/small_record.'+str(i)+'.json' for i in range(8)]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-small-parse-r32-profile-'+str(os.getpid())
...
assert expected[prefix+'r32-rotating-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-rotating-worker'))
...
slug='quiet-'+w.name;cmd=['python3','with_lock.py','--','python3',prefix+'run-small-parse-profile.py','--results-dir','results/'+slug]
...
for n in ['run-small-parse-profile.py','profile-cases.json','workers-provenance.json','build-provenance.json','source.patch','rotating-worker.ts','rotating-worker.js']:shutil.copy2(root/PREFIX/n,d/n)
