...
for row in json.loads((d/'profiles.json').read_text()):
 c=row['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode'];assert row['worker_exit']==row['sampler_exit']==0 and row['stop_reason'] is None and row['diagnostic_only'];assert row['worker_sha256']==sha((w/'r32-rotating-worker').read_bytes())
 assert subprocess.check_output(['/opt/homebrew/bin/node','--version'],text=True).strip()=='v26.5.1'
 cmd=['/opt/homebrew/bin/node',str(w/'rotating-worker.js'),str(bench/'.work/rotating'/c['fixture']),c['operation'],str(c['iterations']),str(c['warmup']),'verify',c['mode']]
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='c695f6509ed85999894a2f34418f057f32bcb363',diagnostic_only=True,note='One frozen R32 small rotating-parse sampling diagnostic; pinned Node executes the complete identical loop and warmup, and checksum plus every VERIFY/LAST/KEEP byte match. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
