...
for ext,data in [('stdout',node.stdout),('stderr',node.stderr)]: (w/('primitive-keys-node.'+ext)).write_bytes(data)
...
for roots,setting in [('native','1'),('shadow','0')]:
 binary=w/(arm+'-primitive-keys-'+roots)
 ...
 cases=[('auto',{},mode,knobs) for mode,knobs in [('normal',{}),('fullgc',{'PERRY_GEN_GC':'0'})]]
 for parser,settings,mode,knobs in cases:
  label=arm+'-primitive-keys-'+roots+'-'+parser+'-'+mode
  try:
  ...
  (w/(arm+'-primitive-keys-validation.json')).write_text(json.dumps(record,indent=2)+'\n')
  print(label,code,row['matches_node'],protected,moved,flush=True)
assert len(record['rows'])==4
if arm=='candidate':
 assert all(r['exit_code']==0 and r['matches_node'] for r in record['rows']), 'Candidate primitive-keys behavior failed; all outcomes preserved.'
