...
lines = [
    '# Native vector escaping: R42', '',
    'R42 is not promoted. Dense escaped pretty printing takes 1,775.855 µs versus 897.777 µs in its immediate parent (+97.81%); the replacer case takes 1,793.242 µs versus 924.234 µs (+94.02%). R42 remains about 3.16×/3.14× Bun on those cases. The ASCII and Unicode controls remain much faster than Node and Bun. Peak RSS changes versus the parent are between −16 and +16 KiB.', '',
    'These are two separately qualified quiet windows, each with seven interleaved R26/Perry/Node/Bun repetitions per case. R39 and R42 were not interleaved against each other. Each window has 168 timed trials and 30 complete-output verification records: 336 timed trials and 60 verifications in total. The six parent measurements are new reference observations for this investigation, additional to the earlier R39 evidence commit.', '',
...
for name, phase in [('R42 candidate', child), ('R39 immediate parent', prior)]:
