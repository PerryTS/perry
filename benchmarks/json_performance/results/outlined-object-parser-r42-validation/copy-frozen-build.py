...
original = other/'benchmarks/json_performance/.work/outlined-object-parser-r42'
...
for p in (original/'initial-cross-worktree-validation').rglob('*'):
    if p.is_file() and p.suffix in allowed and '__pycache__' not in p.parts:
        copy(p, w/'initial-cross-worktree-validation'/p.relative_to(original/'initial-cross-worktree-validation'))
