...
cases=json.loads((w/'profile-cases.json').read_text())['cases'];worker=w/'main-worker';clean={k:v for k,v in os.environ.items() if not k.startswith('PERRY_')};records=[]
for c in cases:
 label=c['fixture']+'-'+c['operation']+'-'+c['mode'];env=clean|({'PERRY_JSON_TAPE':'0'} if c['mode']=='main_direct' else {})
