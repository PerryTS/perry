...
cmd = ['python3', prefix + 'run-focus.py', '--worker', prefix + 'main-worker',
...
(w / 'remote-aa-main-full-command.json').write_text(json.dumps(invocation, indent=2) + '\n')
with (w / 'remote-aa-main-full.log').open('wb') as log:
    result = subprocess.run(['ssh', host, 'cd ' + shlex.quote(remote) + ' && ' + shlex.join(invocation)], stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-aa-main-full.log').read_text(), flush=True)
if result.returncode:
    raise SystemExit(result.returncode)
subprocess.run(['python3', str(w / 'archive-aa.py')], check=True)
