...
cmd = ['python3', prefix + 'run-focus.py', '--worker', prefix + 'control00-worker',
       '--baseline-worker', prefix + 'main-worker', '--node', '/opt/homebrew/bin/node',
       '--bun', '/Users/perry/.bun/bin/bun']
cases = json.loads((w / 'aa-cases.json').read_text())['cases']
...
(w / 'remote-aa-path-full-command.json').write_text(json.dumps(invocation, indent=2) + '\n')
with (w / 'remote-aa-path-full.log').open('wb') as log:
    result = subprocess.run(['ssh', host, 'cd ' + shlex.quote(remote) + ' && ' + shlex.join(invocation)], stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-aa-path-full.log').read_text(), flush=True)
if result.returncode:
    raise SystemExit(result.returncode)
subprocess.run(['python3', str(w / 'archive-aa-path.py')], check=True)
