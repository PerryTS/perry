...
bench = w.parents[1]
d = bench / 'results' / ('quiet-' + w.name + '-aa-main-full')
...
cmd = window['command']; assert cmd[cmd.index('--worker') + 1] == cmd[cmd.index('--baseline-worker') + 1]
...
for (fixture, operation), engines in cases.items():
    ...
    out.append(row); print('A/A', fixture, operation, row['delta_pct'], row['slower_pairs'], 'separated', row['separated'])
(w / 'aa-analysis.json').write_text(json.dumps(dict(window=window, trials=88, verification_trials=10,
    candidate_measured=False, cases=out), indent=2) + '\n')
print('Verified identical-main A/A control: 88 timings and 10 complete-output checks.')
