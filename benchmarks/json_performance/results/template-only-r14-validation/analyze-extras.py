...
for kind in phases:
    ...
    total += len(timings)
assert total == (220 if full_recheck else 2352 if with_full else 196 if options_only else 704 if recheck else 952), total
(work / ('full-recheck-analysis.json' if full_recheck else 'full-analysis.json' if with_full else 'options-analysis.json' if options_only else 'recheck-analysis.json' if recheck else 'extras-analysis.json')).write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
