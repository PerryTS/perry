...
outputs = {}
total = 0
full_recheck = recheck = True
options_only = with_full = False
repetitions = 11
phases = ['full']
for kind in phases:
    ...
    assert len(by_case) == {'access': 12, 'focus': 15, 'options': 4 if recheck else 7, 'full': 7 if full_recheck else 50}[kind]
    ...
    total += len(timings)
assert total == 308, total
original = json.loads((work / 'full-cases.json').read_text())['cases']
fixed = json.loads((work / 'full-recheck-cases.json').read_text())['cases']
for case in outputs['full']['cases']:
    first = next(c for c in original if (c['fixture'], c['operation']) == (case['fixture'], case['operation']))
    count = next(c for c in fixed if (c['fixture'], c['operation']) == (case['fixture'], case['operation']))
    assert all(e['iterations'] == count['iterations'] == 4 * first['iterations'] for e in case['engines'].values())
(work / 'full-recheck-analysis.json').write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
