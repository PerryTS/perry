#!/usr/bin/env python3
"""Compare JSON CPU/RSS across a candidate and up to three reference binaries.
...
def main():
    ...
    parser.add_argument('--previous-worker', type=Path)
    ...
    if args.previous_worker:
        engines['previous'] = [str(args.previous_worker.resolve())]
    meta = dict(started_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
                ...
                sources={p: hashlib.sha256((ROOT / p).read_bytes()).hexdigest()
                         for p in [Path(__file__).name, 'run_dispatch_focus.py', 'worker.js']},
