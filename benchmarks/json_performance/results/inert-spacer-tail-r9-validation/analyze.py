...
phases = ['options'] if options_only else ['access', 'options'] if recheck else ['access', 'focus', 'options']
for kind in phases:
    ...
    assert len(by_case) == {'access': 12, 'focus': 15, 'options': 4 if recheck else 7}[kind]
    ...
    total += len(timings)
assert total == (196 if options_only else 704 if recheck else 952), total
(work / ('options-analysis.json' if options_only else 'recheck-analysis.json' if recheck else 'analysis.json')).write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
