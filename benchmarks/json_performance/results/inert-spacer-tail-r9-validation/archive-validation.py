...
dest = bench / 'results/inert-spacer-tail-r9-validation'
...
manifest = dict(measured_source_commit=json.loads((work / 'provenance.json').read_text())['source_commit'],
                note='R9 separate inert-spacer helper, rejected on ordinary stringify +2.82%. Exactly built candidate and hash-verified reused fresh-main build; workers and main fixtures recompiled at R9 paths. 292 JSON units plus 158 narrow tests pass. Nine native findings remain unsuppressed and match main; shadow/ordinary/callback controls pass. Known lazy and fractional baseline gaps preserved. Script lint public freshness fails; full matrix and CI not claimed.',
                files=entries)
