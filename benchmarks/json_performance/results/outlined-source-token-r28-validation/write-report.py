...
lines=['# Outline source-length construction from small JSON tokens: R28','',(w/'measurement-verdict.md').read_text().strip(),'',
 '## Scope and reference','',
...
def table(title, phase, retained=False):
    ...
    lines.extend(['', '| Fixture / operation | R26 MiB | R28 MiB | Node MiB | Bun MiB | Peak RSS change MiB |',
        '|---|---:|---:|---:|---:|---:|'])
    for row in rows:
        e = row['engines']
        label = row['fixture'] + ' / ' + row.get('operation', row.get('mode', '')) + (f" / {row['count']} live" if retained else '')
    ...
    if retained:
        lines.extend(['', '| Fixture / operation | R26 after MiB | R28 after MiB | Node after MiB | Bun after MiB | After-RSS change MiB |', '|---|---:|---:|---:|---:|---:|'])
        for row in rows:
            e = row['engines']
...
lines.extend(['## Build fingerprints','','| Artifact | R26 SHA-256 | R28 SHA-256 |','|---|---|---|'])
...
lines.extend(['','The artifact index records archived raw windows, exact sample vectors, validation receipts, source patches and failed attempts. Independent R26 stringify profiles and future scanner/emitter proposals, if included, are diagnostic/proposal evidence and are not R28 performance results.',''])
assert len(lines)<1900
(bench/'OUTLINED_SOURCE_TOKEN_R28.md').write_text('\n'.join(lines))
print('Wrote R28 report with',len(lines),'lines')
