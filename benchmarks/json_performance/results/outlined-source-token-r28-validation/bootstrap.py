...
w=Path(__file__).resolve().parent;root=w.parents[3];old=w.with_name('source-length-hint-r27');sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()
assert head=='decc2d9eed2de5713b800ddaf77ffc75c78fdd0c'
...
for p in old.rglob('*'):
 ...
 if first.startswith('main-') and first!='main-reuse-provenance.json':copy(p,w/rel)
...
# Branch from R27 source before its evidence commit. This R26 evidence base
# makes the measured source patch include BOTH length reuse and outlining,
# without including R27's separately archived trial payloads in the patch.
base='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d'
(w/'base.json').write_text(json.dumps(dict(base_commit=base,reference_name='R26',branch_parent_source=head,previous_candidate_evidence='0ea4c4833080af52a48a7c458a92b14aee0dedfc'),indent=2)+'\n')
p=w/'run-after-build.py';p.write_text(p.read_text().replace('R27 behavior','R28 behavior'))
p=w/'complete-candidate-validation.py';p.write_text(p.read_text().replace('R27 candidate','R28 candidate'))
p=w/'stage.py';p.write_text(p.read_text().replace('json-r27-stage-','json-r28-stage-'))
p=w/'compare-json-entry-disassembly.py';p.write_text(p.read_text().replace("'r27':w/'candidate-worker'","'r28':w/'candidate-worker'"))
for r in records:
...
(w/'main-reuse-provenance.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',reference_evidence_commit='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d',copy_via_r27_evidence='0ea4c4833080af52a48a7c458a92b14aee0dedfc',note='Exact frozen R26 reference copied from R27 main arm. All 90 reference behavior receipts and 26 IR verdicts are reused. Reference is not current main or R27 candidate.',files=records),indent=2)+'\n')
print('VERIFIED',len(records),'R26 reference/controller payloads via R27; no production edit.')
