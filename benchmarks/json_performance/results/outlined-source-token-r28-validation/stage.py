...
subprocess.run(['python3',str(w/'compare-token.py')],cwd=root,check=True)
...
files += [str((w/name).relative_to(bench)) for name in ['main-token-validation.json','candidate-token-validation.json','main-token-roots.json','candidate-token-roots.json','token-root-comparison.json']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in files};token='json-r28-stage-'+str(os.getpid())
