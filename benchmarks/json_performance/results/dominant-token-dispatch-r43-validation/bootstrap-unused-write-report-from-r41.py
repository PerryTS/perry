...
lines = [
    ...
    'Known native-root findings, lazy descriptor and fractional-spacing findings, and frozen-reference emitter output/moving-GC failures remain explicit. Their receipts are retained; this is not a blanket conformance pass. R43 has no alternate-worktree validation refusal. The earlier R40 refusal is documented in its own evidence.', '',
...
lines += ['', '“Separated” means the smallest candidate sample exceeds the largest reference sample. It is a descriptive screen, not a significance test. Overlapping samples do not prove equivalence. These R43 rows have not received a second independent R43 recheck.', '']
...
lines += ['', '## Earlier scalar writer and profile follow-up', '',
    'The prior scalar writer in R39 measured 897.777 µs for escaped pretty printing and 924.234 µs for the replacer case. R43 reduces those medians by 62.49% and 60.65%. These are separately qualified windows, not paired R39/R43 trials. R39 is the earlier scalar implementation; the immediate source parent is the rejected R40 vector writer. The R39 control window and raw samples are in [R40 evidence](https://github.com/PerryTS/perry/blob/df1c4e18edc15ea37ec294a2b7c25af070001276/benchmarks/json_performance/NATIVE_VECTOR_ESCAPE_R40.md). They are not counted as new R43 trials.', '',
