...
slug = 'quiet-' + w.name + '-full-retry1'
invocation = ['python3', 'with_lock.py', '--'] + cmd + ['--results-dir', 'results/' + slug]
(w / 'remote-full-retry1-command.json').write_text(json.dumps(invocation, indent=2) + '\n')
with (w / 'remote-full-retry1.log').open('wb') as log:
    result = subprocess.run(['ssh', host, 'cd ' + shlex.quote(remote) + ' && ' + shlex.join(invocation)], stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-full-retry1.log').read_text(), flush=True)
subprocess.run(['python3', str(w / 'archive-results.py'), slug, 'full', '--allow-failed-window'], check=True)
d = w.parents[1] / 'results' / slug
for name in ['remote-full-retry1-command.json', 'remote-full-retry1.log', 'remote-stage-hashes.json', 'full-cases.json']:
