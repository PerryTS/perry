...
for source in sorted(work.rglob('*')):
    ...
    compressed = ('korean-fixtures' in source.parts and source.name != 'manifest.json') or source.suffix in {'.log', '.stdout', '.stderr', '.ll', '.s', '.patch', '.diff'} or source.name.endswith('.sample.txt') or source.name in {'callback-only.ts', 'run-zero-retained.py'}
