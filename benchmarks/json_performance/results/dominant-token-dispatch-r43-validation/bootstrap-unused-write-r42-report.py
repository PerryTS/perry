...
lines = [
    '# Outlined object parsing: R43', '',
    'Outlining the large object parser partially reduces the wide-object parse regression. R43 takes 2,977.0625 µs, 3.19% above its interleaved R26 reference; the previous R41 screen measured a 4.29% gap. R43 remains unpromoted: seven of the twelve targeted rows have separated CPU regressions versus R26.', '',
    'One qualified quiet window contains 528 timed trials and 60 complete-output verification records, with eleven interleaved R26/Perry/Node/Bun repetitions per case. The twelve cases cover all ten regressions from the qualified R41 screen plus two long-string stringify controls. Every case was declared before R43 timing. Three additional rows were added to the initial nine-case declaration after R41 identified them; iterations and warmup remain unchanged.', '',
...
lines += ['', '“Separated” means the smallest candidate sample exceeds the largest reference sample. It is a descriptive screen, not a significance test; overlapping ranges do not prove equivalence. R43 has no second independent timing recheck.', '',
          '## Peak RSS', '', '| Fixture / operation | R26 MiB | R43 MiB | Node MiB | Bun MiB |', '|---|---:|---:|---:|---:|']
for c in cases:
    e = c['engines']
...
lines += ['', '## Immediate parent control', '',
    'These R41 and R43 observations come from separate quiet windows, not interleaved R41/R43 pairs. R41 used seven repetitions per case and R43 used eleven. The parent window is historical evidence from [R41](https://github.com/PerryTS/perry/blob/b5bb7ecd299e9c9513964dcc423e3c33e46fa4d4/benchmarks/json_performance/PACKED_VECTOR_ESCAPE_R41.md), not an additional R43 trial count.', '',
    '| Fixture / operation | R41 µs | R43 µs | Difference |', '|---|---:|---:|---:|']
for c in cases:
...
lines += ['', '## Storage recovery and provenance', '',
    ...
    'The next experiment uses a bounded prefix check to reserve the long-token specialization for potentially useful documents. R43 alone does not meet the no-regression objective.', '']
