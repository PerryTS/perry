...
def table(title, phase, retained=False):
    ...
    lines.extend(['### ' + title, '', f"Window: {window['started_utc']} to {window['finished_utc']}.", '',
        '| Fixture / operation | R26 µs | R43 µs | Node µs | Bun µs | CPU change | Ranges |',
        '|---|---:|---:|---:|---:|---:|---|'])
    for row in rows:
        e = row['engines']
    ...
    lines.extend(['', '| Fixture / operation | R26 MiB | R43 MiB | Node MiB | Bun MiB | Peak RSS change MiB |',
        '|---|---:|---:|---:|---:|---:|'])
    for row in rows:
        e = row['engines']
        label = row['fixture'] + ' / ' + row.get('operation', row.get('mode', '')) + (f" / {row['count']} live" if retained else '')
    ...
    if retained:
        lines.extend(['', '| Fixture / operation | R26 after MiB | R43 after MiB | Node after MiB | Bun after MiB | After-RSS change MiB |', '|---|---:|---:|---:|---:|---:|'])
        for row in rows:
            e = row['engines']
...
lines.extend(['## Build fingerprints','','| Artifact | R26 SHA-256 | R43 SHA-256 |','|---|---|---|'])
