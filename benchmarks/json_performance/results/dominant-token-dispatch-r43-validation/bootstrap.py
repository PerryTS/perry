...
old = primary / 'benchmarks/json_performance/.work/outlined-object-parser-r42'
...
def copy(source, destination, adapt=False):
    ...
    data = raw
    if adapt:
        data = raw.decode().replace('outlined-object-parser-r42', w.name).replace('OUTLINED_OBJECT_PARSER_R42', 'DOMINANT_TOKEN_DISPATCH_R43').replace('R42', 'R43').replace("'r42'", "'r43'").replace('json-r42-', 'json-r43-').replace('b9bfe3070f90f223e1d9d3526df54beb57308328', head).encode()
