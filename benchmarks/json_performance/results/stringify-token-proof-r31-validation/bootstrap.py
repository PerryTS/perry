...
w=Path(__file__).resolve().parent;root=w.parents[3];old=w.with_name('stringify-emitter-flags-r30');sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()
assert head=='f0bc3aeb6a2597a941ad4afe1b70e79e2df11a17'
...
(w/'base.json').write_text(json.dumps(dict(base_commit=base,reference_name='R26',branch_parent_source=head,previous_candidate_source='3c14d729947343431cc98e18fd78a3f06cd5f538'),indent=2)+'\n')
...
print('VERIFIED',len(records),'R26 reference/controller payloads via R30; no production edit.')
