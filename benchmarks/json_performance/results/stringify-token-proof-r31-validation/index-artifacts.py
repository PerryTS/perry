...
if '--write' in sys.argv:
 ...
 paths=[bench/'STRINGIFY_TOKEN_PROOF_R31.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 assert len(paths)==len(set(paths))
 timed=verify=calibration=0
 for d in dirs[:-1]:
  terminal=json.loads((d/'controller-exit.json').read_text())
  if terminal['exit_code']!=0:
   assert d.name=='quiet-stringify-token-proof-r31-small-options-recheck-options'
   assert terminal['exit_code']==1 and terminal['measurement_command_success_proven']
   assert sha((d/'with_lock.py').read_bytes())==terminal['with_lock_sha256']
   assert sha((d/'remote-small-options-recheck-options.log').read_bytes())==terminal['remote_log_sha256']
 ...
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=timed,verification_records=verify,calibration_trials=calibration,qualification='Not promoted: persistent small callback and fresh Unicode parse regressions. Controlled R26 comparison at0.5.1531; local archive-copy failure preserved separately from completed remote measurement. Not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
