...
staged=read('remote-profile-stage-hashes.json');assert staged==json.loads((w/'remote-profile-stage-hashes.json').read_text()) and len(staged)==105
for path,digest in staged.items():assert sha(bench/path)==digest,path
oracle={}
for line in (bench/'results'/('quiet-'+w.name+'-screen-focus')/'verify.jsonl').read_text().splitlines():
 r=json.loads(line)
 if r['engine']=='node':oracle[r['fixture'],r['operation']]=r
rows=read('profiles.json');assert len(rows)==2;out=[]
for r in rows:
 c=r['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode'];assert r['worker_exit']==r['sampler_exit']==0 and r['stop_reason'] is None;assert r['worker_sha256']==sha(w/(c['mode']+'-worker'));assert r['worker_command'][-1]=='verify';assert r['observed_peak_rss_bytes']<=2*1024**3
 stdout=read_text(d/(label+'.stdout'));values=list(map(float,re.search(r'^RESULT (.+)$',stdout,re.M)[1].split()));assert len(values)==7;expected=oracle[c['fixture'],c['operation']];assert values[5]==expected['checksum']/(expected['iterations']+expected['warmup'])*(c['iterations']+c['warmup']) and values[6]==0
 ...
 phases={'tape_record_producer':phase(lambda n:'materialize_tape_records' in n),'tape_build':phase(lambda n:'build_tape_into' in n),'lazy_full_materialization':phase(lambda n:'force_materialize_lazy' in n),'direct_array_parser':phase(lambda n:'DirectParser11parse_array ' in n),'collection':phase(lambda n:'gc_collect_full_mark_sweep_with_trigger' in n or 'gc_collect_minor_with_trigger_inner' in n)}
 assert (phases['tape_record_producer']['samples']>0)==(c['mode']=='candidate'),(c,phases)
 for value in phases.values():assert value['samples']<=total;value['pct_of_main_thread_samples']=value['samples']/total*100
 out.append({'case':c,'samples':total,'profile_sha256':hashlib.sha256(report.encode()).hexdigest(),'full_output_matches_node':True,'phases':phases,'observed_peak_rss_mib':r['observed_peak_rss_bytes']/1048576,'note':'One short instrumented sample, not an isolated phase timer. Tape producer/direct parser are nested under full materialization; overlapping columns must not be added.'})
...
print('VERIFIED two complete outputs against Node, positive candidate producer entry, sampling coverage, all 105 hashes and quiet window.',flush=True)
