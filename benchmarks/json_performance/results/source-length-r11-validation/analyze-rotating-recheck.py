...
assert len(timing) == 44 and len(calibration) == 4 and len(verify) == 12
...
assert len(output) == 1
(w / 'rotating-recheck-analysis.json').write_text(json.dumps(dict(trials=44, calibration_trials=4,
    verification_trials=12, window=window, cases=output), indent=2) + '\n')
print('Verified all 44 timing, 4 calibration, 12 full-output checks, hashes and quiet window.')
