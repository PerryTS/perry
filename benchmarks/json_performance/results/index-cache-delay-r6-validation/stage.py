...
assert all(x['matches_node'] and x['exit_code']==0 for x in json.loads((work/'dispatch-validation.json').read_text()))
...
token='json-r6-stage-'+str(os.getpid())
def ssh(code):
...
try:
    ...
    files+=['.work/'+work.name+'/'+n for n in ['worker','access-worker','rotating-worker',
            'provenance.json','source.patch','gc-witness.json','codegen-comparison.json',
            'run-access.py','build-provenance.json']]
    ...
    for name in ['worker','access-worker']:
        path='.work/invariant-field-loop-r5/'+name
        expected[path]=hashlib.sha256((bench/path).read_bytes()).hexdigest()
