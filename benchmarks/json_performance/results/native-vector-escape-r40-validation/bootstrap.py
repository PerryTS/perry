...
old = primary / 'benchmarks/json_performance/.work/compact-template-capture-r39'
head = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=root, text=True).strip()
assert head == '3bde9394e021039603c84801762af4f083b1f9c4'
...
def copy(source, destination, adapt=False):
    ...
    data = raw
    if adapt:
        data = raw.decode().replace('compact-template-capture-r39', w.name).replace('COMPACT_TEMPLATE_CAPTURE_R39', 'NATIVE_VECTOR_ESCAPE_R40').replace('R39', 'R40').replace("'r39'", "'r40'").replace('json-r39-', 'json-r40-').replace('e69d1292110cf9abee24f8690c584652805a4167', head).encode()
