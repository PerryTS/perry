...
os.chdir(root)
b=Path('benchmarks/json_performance'); work=b/'.work/lazy-canonical-r5'
assert '306 passed; 0 failed' in Path('/tmp/json-lazy-canonical-r5-tests.log').read_text()
assert 'json_small_object_template_survives_evacuation ... ok' in Path('/tmp/json-lazy-canonical-r5-tests.log').read_text()
assert 'Finished `release`' in Path('/tmp/json-lazy-canonical-r5-build.log').read_text()
tested=json.loads((b/'.work/lazy-canonical-r5-tests.json').read_text())
