...
os.chdir(root)
b=Path('benchmarks/json_performance'); work=b/'.work/lazy-canonical-r5'; dest=work/'canonical-probe'
...
keys = ['a' * n for n in [0, 1, 2, 63, 64, 65, 127, 128, 129]] + ['é', '😀']
extras['key_length_buckets'] = '{' + ','.join(json.dumps(k, ensure_ascii=False) + ':' + str(i) for i,k in enumerate(keys)) + '}'
long_key = 'a' * 64
extras['key_length_collision_duplicates'] = '{' + json.dumps(long_key) + ':1,' + json.dumps('b' * 128) + ':2,' + json.dumps(long_key) + ':3}'
extras['key_length_nested'] = r'{"x":{"x":1,"y":2},"y":[{"x":3},{"y":4}],"zz":5}'
extras['key_length_32_width'] = '{' + ','.join(json.dumps('a' * n) + ':' + str(n) for n in range(32)) + '}'
