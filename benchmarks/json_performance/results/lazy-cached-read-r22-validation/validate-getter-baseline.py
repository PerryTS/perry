...
w=Path(__file__).resolve().parent;root=w.parents[3];clean={k:v for k,v in os.environ.items() if not k.startswith('PERRY_')};source=w/'initial-validation/test_json_cached_reads.ts';records=[]
