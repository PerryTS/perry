...
cases = json.loads((w / 'screen-cases.json').read_text())['cases']
assert len(cases) == 18 and all(c['repetitions'] == 7 for c in cases)
for c in cases:
    cmd += ['--case', ':'.join(str(c[k]) for k in ['fixture', 'operation', 'iterations', 'warmup', 'repetitions'])]
slug = 'quiet-' + w.name + '-screen-focus'
invocation = ['python3', 'with_lock.py', '--'] + cmd + ['--results-dir', 'results/' + slug]
(w / 'remote-screen-focus-command.json').write_text(json.dumps(invocation, indent=2) + '\n')
with (w / 'remote-screen-focus.log').open('wb') as log:
    result = subprocess.run(['ssh', host, 'cd ' + shlex.quote(remote) + ' && ' + shlex.join(invocation)], stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-screen-focus.log').read_text(), flush=True)
subprocess.run(['python3', str(w / 'archive-results.py'), slug, 'focus', '--allow-failed-window'], check=True)
d = w.parents[1] / 'results' / slug
for name in ['remote-screen-focus-command.json', 'remote-screen-focus.log', 'remote-stage-hashes.json', 'screen-cases.json']:
