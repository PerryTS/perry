...
for row in rotating:
 if row['fixture'] in wanted:
  for i,digest in enumerate(row['sha256']):
   ...
   files.append(p)
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in files};token='json-r22-stage-'+str(os.getpid())
