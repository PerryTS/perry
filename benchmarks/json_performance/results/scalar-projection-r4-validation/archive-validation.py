...
dest=bench/'results/scalar-projection-r4-validation'
dest.mkdir(exist_ok=False)
entries=[]
allowed={'.py','.json','.ts','.log','.stdout','.stderr','.ll','.md','.rs','.txt','.diff'}
...
manifest=dict(measured_source_commit=json.loads((work/'provenance.json').read_text())['source_commit'],
              note='Measured binaries are frozen at this commit. R3 separately establishes the inherited native fixture findings on actual main; this archive covers R4.',files=entries)
