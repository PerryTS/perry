...
token='json-r4-stage-'+str(os.getpid())
def ssh(code):
...
try:
    ...
    files+=['.work/scalar-projection-r4/'+n for n in ['worker','access-worker','rotating-worker',
            'provenance.json','source.patch','gc-witness.json','codegen-comparison.json','main-codegen-r3-runtime-worker','hybrid-provenance.json']]
