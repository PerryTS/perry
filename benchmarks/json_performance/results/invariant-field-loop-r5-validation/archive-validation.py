...
dest=bench/'results/invariant-field-loop-r5-validation'
...
manifest=dict(measured_source_commit=json.loads((work/'provenance.json').read_text())['source_commit'],
              note='R5 measured binaries are frozen at this commit. All four unsuppressed native fixture fingerprints are independently reproduced by actual main53df on these exact fixtures. unit-verified.log and json-unit.log cover final source; earlier failed/interim test logs are retained and are not final-source proof. No full CI or full-matrix claim.',files=entries)
