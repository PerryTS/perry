...
w=Path(__file__).resolve().parent;bench=w.parents[1];host='perry@perry-macos.local';remote='/Users/perry/json-codex-yHdsko/benchmarks/json_performance';expected=json.loads((w/'remote-stage-hashes.json').read_text());files=[str((w/n).relative_to(bench)) for n in ['run-profiles.py','profile-cases.json']]
...
token='json-r17-profile-stage-'+str(os.getpid())
...
try:
 ...
 ssh('from pathlib import Path;import hashlib;r=Path('+repr(remote)+');e='+repr(expected)+';assert {p:hashlib.sha256((r/p).read_bytes()).hexdigest() for p in e}==e;print("VERIFIED",len(e),"profile input hashes")')
