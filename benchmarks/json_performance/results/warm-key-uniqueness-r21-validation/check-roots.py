...
sources=[w/(stem+'.ts') for stem in ['test_json_source_length','test_json_template_capture','test_json_cached_construction','test_json_warm_unique_keys','prior-entry','callback-only','options-worker']]+[w/'harness'/(stem+'.ts') for stem in ['worker','access-worker']];results=[]
