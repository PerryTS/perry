...
cases = json.loads((w / 'controls-cases.json').read_text())['cases']
...
slug = 'quiet-' + w.name + '-controls-focus'
invocation = ['python3', 'with_lock.py', '--'] + cmd + ['--results-dir', 'results/' + slug]
(w / 'remote-controls-focus-command.json').write_text(json.dumps(invocation, indent=2) + '\n')
with (w / 'remote-controls-focus.log').open('wb') as log:
    result = subprocess.run(['ssh', host, 'cd ' + shlex.quote(remote) + ' && ' + shlex.join(invocation)], stdout=log, stderr=subprocess.STDOUT)
print((w / 'remote-controls-focus.log').read_text(), flush=True)
subprocess.run(['python3', str(w / 'archive-results.py'), slug, 'focus', '--allow-failed-window'], check=True)
d = w.parents[1] / 'results' / slug
for name in ['remote-controls-focus-command.json', 'remote-controls-focus.log', 'remote-stage-hashes.json', 'controls-cases.json']:
