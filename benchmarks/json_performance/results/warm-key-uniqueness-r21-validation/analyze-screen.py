...
phases = ['focus']
for kind in phases:
    d = bench / 'results' / ('quiet-' + work.name + '-screen-' + kind)
    ...
    assert len(verifies) == 90 and len(timings) == 504 and len(summary) == 72
    ...
    declarations = {(c['fixture'], c['operation']): c for c in json.loads((work / 'screen-cases.json').read_text())['cases']}
    ...
    assert len(by_case) == {'access': 12, 'focus': 18, 'options': 4 if recheck else 7, 'full': 5 if full_recheck else 50}[kind]
    ...
    total += len(timings)
assert total == 504, total
(work / 'screen-analysis.json').write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
