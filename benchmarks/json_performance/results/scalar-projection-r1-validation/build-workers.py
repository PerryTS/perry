...
env = clean | {'PERRY_RUNTIME_DIR':str(root/'target/release')}
commands = []
for name in ['worker', 'rotating-worker']:
    source = root/'benchmarks/json_performance'/(name+'.ts')
    obj = work/(name+'.o')
    binary = work/name
    cmd = [str(root/'target/release/perry'), 'compile', str(source),
    ...
    cmd = ['cc',str(obj),str(root/'target/release/libperry_runtime.a'),'-lc',
           '-Wl,-dead_strip','-Wl,-no_exported_symbols','-o',str(binary)]
    commands.append(cmd)
    with (work/(name+'-link.log')).open('wb') as log:
        subprocess.run(cmd,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=180)
    print('COMPILED',name,flush=True)
paths = [root/'target/release'/n for n in ['perry','libperry_runtime.a','libperry_stdlib.a']]
paths += [root/'benchmarks/json_performance'/(n+'.ts') for n in ['worker','rotating-worker']]
paths += [work/n for n in ['worker','worker.o','rotating-worker','rotating-worker.o']]
meta = dict(source_commit=commit, base_commit=base, commands=commands,
            runtime_dir=str(root/'target/release'),
