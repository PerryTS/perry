...
for mode, rs4gc in ([('shadow','0')] if '--shadow-only' in sys.argv else [('native','1'), ('shadow','0')]):
    ...
    for source in sources:
        ...
        assert calls > 0, (source,mode,'probe not emitted')
        counts.append(dict(source=str(source.relative_to(root)),lowering=mode,probe_calls=calls))
    ...
    for index, flags in enumerate(variants):
        ...
        assert result.returncode==0,(mode,index,(output/('check-'+str(index)+'.log')).read_text()[-4000:])
        print('PASS roots',mode,index,flush=True)
(work/('shadow-codegen-comparison.json' if '--shadow-only' in sys.argv else 'codegen-comparison.json')).write_text(json.dumps(dict(probe_counts=counts,statepoint_passes=passes,
