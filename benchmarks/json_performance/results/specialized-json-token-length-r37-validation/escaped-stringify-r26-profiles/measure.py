...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-stringify-profiles.py','profile-cases.json','r26-options-worker','workers-provenance.json','build-provenance.json','source.patch','options-worker.ts','options-worker.js']]+['.work/fixtures/'+f+'.json' for f in ['escaped_1m']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-escaped-stringify-r26-profiles-'+str(os.getpid())
assert json.loads((w/'build-provenance.json').read_text())['source_commit']=='3aac4d6335da54abeeed73df842decbbe6dd5d71'
proof=next(r for r in json.loads((w/'workers-provenance.json').read_text()) if r['worker']=='options')
assert expected[prefix+'r26-options-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-options'))
...
slug='quiet-'+w.name;cmd=['python3','with_lock.py','--','python3',prefix+'run-stringify-profiles.py','--results-dir','results/'+slug]
...
for n in ['run-stringify-profiles.py','profile-cases.json','workers-provenance.json','build-provenance.json','source.patch','options-worker.ts','options-worker.js']:shutil.copy2(root/PREFIX/n,d/n)
'''.replace('REMOTE',repr(remote)).replace('SLUG',repr(slug)).replace('PREFIX',repr(prefix))
ssh(code)
# Archive locally under ignored WORK while the independent parse build requires a clean source tree.
