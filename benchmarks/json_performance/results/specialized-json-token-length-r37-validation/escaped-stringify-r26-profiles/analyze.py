...
for row in json.loads((d/'profiles.json').read_text()):
 ...
 cmd=['/opt/homebrew/bin/node',str(w/'options-worker.js'),str(bench/'.work/fixtures'/(c['fixture']+'.json')),c['operation'],str(c['iterations']),str(c['warmup']),'verify']
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',diagnostic_only=True,note='Two frozen R26 escaped-string stringify diagnostics. Pinned Node executes the complete identical iteration and warmup counts; full output and checksum match. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
