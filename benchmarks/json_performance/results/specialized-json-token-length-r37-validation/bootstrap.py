...
w=Path(__file__).resolve().parent;root=w.parents[3];old=w.with_name('isolated-json-scan-chunks-r36');sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()
assert head=='3d59ed5219ace6bf001df3de9108c52434dc4232'
...
def copy(p,d,adapt=False):
 ...
 if adapt:
  d.write_text(p.read_text().replace('isolated-json-scan-chunks-r36','specialized-json-token-length-r37').replace('ISOLATED_JSON_SCAN_CHUNKS_R36','SPECIALIZED_JSON_TOKEN_LENGTH_R37').replace('R36','R37').replace("'r36'","'r37'").replace('json-r36-','json-r37-'))
...
(w/'base.json').write_text(json.dumps(dict(base_commit='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d',reference_name='R26',branch_parent_source=head,previous_candidate_source='2f7c36be28acc9e090d4aef1c2337335d83ee6b0'),indent=2)+'\n')
(w/'main-reuse-provenance.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',reference_evidence_commit='a510f0fcb7c38670d82b10f3f074a4b66b1aef4d',copy_via_r36_evidence='c68c824770cf6e0bc52f93bd7294361188a12398',note='Exact frozen R26 reference copied through R36. Reuse81original plus20emitter plus4primitive reference executions and28IR only with source/fixture equivalence. Primitive native finding is preserved unsuppressed; shadowchecks pass. All candidate executions will be fresh. No current-main or R36-candidate reference.',files=records),indent=2)+'\n')
