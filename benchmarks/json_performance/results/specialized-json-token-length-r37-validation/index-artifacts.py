...
if '--write' in sys.argv:
 ...
 paths=[bench/'SPECIALIZED_JSON_TOKEN_LENGTH_R37.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=timed,verification_records=verify,calibration_trials=calibration,qualification='Not promoted: escaped 1 MB fresh/repeated regressions of 8.34%/8.51%. Fresh Unicode and Korean improve; small fresh +0.49% has overlapping ranges and requires recheck if otherwise qualifying. Two rotating windows only; no current-source options/full-matrix performance. Frozen R26 reference at 0.5.1531; not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
