...
original=original_root/'benchmarks/json_performance/.work/specialized-json-token-length-r37'
...
strings=json.loads((original/'string-suite-provenance.json').read_text());assert strings['source_commit']==head and strings['exit_code']==0
...
for n in ['build-provenance.json','build-start.json','build-command-result.json','unit-source.json','unit.log','build.log','validate-and-build.log','lint-results.json','script-lint.log','file-cap.log','worktree-provenance.json','lint-review.json','string-suite.log','string-suite-provenance.json','build-resume.json','build-controller.log','hold-production-reviewed.txt','format-only.patch','format-after.log']:
 copy(original/n,w/n)
for n in ['build-release.py','validate-and-build.py','run-lint.py']:
 ...
 copy(original/n,p)
for name in ['initial-77ea-preproduction','initial-599b-formatting']:
 for p in (original/name).rglob('*'):
  if p.is_file():copy(p,w/name/p.relative_to(original/name))
