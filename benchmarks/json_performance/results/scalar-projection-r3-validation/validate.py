...
env = clean | {'PERRY_RUNTIME_DIR': str(root/'target/release'), 'PERRY_NO_AUTO_OPTIMIZE': '1'}
...
inputs = [('fixture', root/'test-files/test_json_scalar_projection.ts'),
...
source = inputs[0][1]
expected = invoke(['/opt/homebrew/bin/node', '--experimental-strip-types', str(source)], 'candidate-node-oracle', clean)
...
for mode, settings in [('auto', {}), ('tape', {'PERRY_JSON_TAPE': '1'}), ('direct', {'PERRY_JSON_TAPE': '0'})]:
    for gc, knobs in [('normal', {}), ('scheduled', stress), ('fullgc', {'PERRY_GEN_GC': '0'})]:
        label = 'candidate-'+mode+'-'+gc
        output = invoke([str(work/'candidate-fixture')], label,
                        clean | settings | knobs | {'PERRY_GC_DIAG': '1'})
        ...
        matrix.append(dict(mode=mode, gc=gc, matches_node=True,
...
paths = [compiler, root/'target/release/libperry_runtime.a', root/'target/release/libperry_stdlib.a']
