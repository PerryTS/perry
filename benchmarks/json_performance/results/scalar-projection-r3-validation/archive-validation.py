...
dest=bench/'results/scalar-projection-r3-validation'
dest.mkdir(exist_ok=False)
entries=[]
allowed={'.py','.json','.ts','.log','.stdout','.stderr','.ll','.md','.rs','.txt'}
...
manifest=dict(measured_source_commit=json.loads((work/'provenance.json').read_text())['source_commit'],
              note='Measured binaries are frozen at this commit. Native main reproduction evidence is included with unchanged checker and input hashes.',files=entries)
