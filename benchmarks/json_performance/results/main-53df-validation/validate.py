...
os.chdir(root)
b=Path('benchmarks/json_performance'); work=b/'.work/main-53df'
assert '291 passed; 0 failed' in Path('benchmarks/json_performance/results/tape-depth-r2-validation/tests.log').read_text()
assert 'json_small_object_template_survives_evacuation ... ok' in Path('benchmarks/json_performance/results/tape-depth-r2-validation/tests.log').read_text()
assert 'Finished `release`' in Path('/tmp/json-main-53df-build.log').read_text()
tested=json.loads((b/'.work/main-53df-freeze.json').read_text())
assert subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()==tested['base_commit']=='53df2c671fff33b1f8f624432372c2401db8b1d0'
assert not subprocess.check_output(['git','diff','df64624c8c7b08ca09e78f0006468f3d0befed67','HEAD','--','crates/perry-runtime'],text=True)
