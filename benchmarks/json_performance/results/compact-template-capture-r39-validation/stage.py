...
assert json.loads((w/'candidate-resume-result.json').read_text())['exit_code']==0
for path,digest in json.loads((w/'fresh-root-input-proof.json').read_text())['verified_hashes'].items():assert hashlib.sha256((root/path).read_bytes()).hexdigest()==digest,path
...
strings=json.loads((w/'string-suite-provenance.json').read_text());assert strings['source_commit']==head and strings['exit_code']==0
...
subprocess.run(['python3',str(w/'compare-emitters.py')],cwd=root,check=True)
subprocess.run(['python3',str(w/'compare-primitive-keys.py')],cwd=root,check=True)
subprocess.run(['python3',str(w/'compare-large-emitters.py')],cwd=root,check=True)
...
files += [str((w/name).relative_to(bench)) for name in ['main-emitter-validation.json','candidate-emitter-validation.json','main-emitter-roots.json','candidate-emitter-roots.json','emitter-comparison.json']]
files += [str((w/name).relative_to(bench)) for name in ['build-copy-provenance.json','string-suite-provenance.json']]
files += [str((w/name).relative_to(bench)) for name in ['main-primitive-keys-validation.json','candidate-primitive-keys-validation.json','main-primitive-keys-roots.json','candidate-primitive-keys-roots.json','primitive-keys-comparison.json']]
files += [str((w/name).relative_to(bench)) for name in ['main-large-emitter-validation.json','candidate-large-emitter-validation.json','main-large-emitter-roots.json','candidate-large-emitter-roots.json','large-emitter-comparison.json']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in files};token='json-r39-stage-'+str(os.getpid())
