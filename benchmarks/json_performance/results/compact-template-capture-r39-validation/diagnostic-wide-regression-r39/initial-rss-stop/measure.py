...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-parse-regression-profile.py','profile-cases.json','r39-worker','r26-worker','workers-provenance.json','main-workers-provenance.json','build-provenance.json','main-build-provenance.json','source.patch','worker.ts','worker.js']]+['.work/fixtures/'+f+'.json' for f in ['wide_1m','records_object_8m']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-parse-regression-r39-profile-'+str(os.getpid())
assert json.loads((w/'build-provenance.json').read_text())['source_commit']=='e69d1292110cf9abee24f8690c584652805a4167'
proof=next(r for r in json.loads((w/'workers-provenance.json').read_text()) if r['worker']=='worker')
assert expected[prefix+'r39-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-worker'))
assert json.loads((w/'main-build-provenance.json').read_text())['source_commit']=='3aac4d6335da54abeeed73df842decbbe6dd5d71'
reference=next(r for r in json.loads((w/'main-workers-provenance.json').read_text()) if r['worker']=='worker')
assert expected[prefix+'r26-worker']==next(h for p,h in reference['files'].items() if p=='benchmarks/json_performance/.work/trusted-materialized-read-r26/candidate-worker')
...
slug='quiet-'+w.name;cmd=['python3','with_lock.py','--','python3',prefix+'run-parse-regression-profile.py','--results-dir','results/'+slug]
...
for n in ['run-parse-regression-profile.py','profile-cases.json','workers-provenance.json','main-workers-provenance.json','build-provenance.json','main-build-provenance.json','source.patch','worker.ts','worker.js']:shutil.copy2(root/PREFIX/n,d/n)
'''.replace('REMOTE',repr(remote)).replace('SLUG',repr(slug)).replace('PREFIX',repr(prefix))
ssh(code)
# Archive locally under ignored WORK while the independent R34 validation requires a clean source tree.
