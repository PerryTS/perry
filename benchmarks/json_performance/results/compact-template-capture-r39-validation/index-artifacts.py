...
if '--write' in sys.argv:
 ...
 paths=[bench/'COMPACT_TEMPLATE_CAPTURE_R39.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 for d in dirs[:-1]:
  ...
  window=json.loads((d/'window.json').read_text());quiet=window['quiet_gate_passed'];windows.append(dict(path=str(d.relative_to(root)),qualified=quiet,started_utc=window['started_utc'],finished_utc=window['finished_utc']))
 ...
 assert (timed,verify,calibration)==(2348,415,72)
 assert qualified==dict(timed=2348,verify=415,calibration=72)
 m=json.loads((w/'provenance.json').read_text())
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=qualified['timed'],verification_records=qualified['verify'],calibration_trials=qualified['calibration'],archived_trial_counts=dict(timed=timed,verify=verify,calibration=calibration),windows=windows,qualification='Not promoted: seven full-screen regressions recur in an independent recheck, including wide parse +4.51% and wide stringify +2.32%. Fresh small parse improves 2.78% in its recheck. Four qualified windows, 77 comparisons. No options/Korean/changing/retained/access timing. Diagnostic profile failures and their archive limitation are retained separately. Frozen R26 reference, not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
