...
sources=[w/(stem+'.ts') for stem in ['test_json_source_length','test_json_template_capture','prior-entry','callback-only','options-worker']]+[w/'harness'/(stem+'.ts') for stem in ['worker','access-worker']];results=[]
for mode,rs4gc in [('native','1'),('shadow','0')]:
 ...
 variants=[['--statepoints','--max-stale','0','--min-files','7','--min-statepoints','1','--min-live-bundles','1','--min-relocates','1']] if mode=='native' else [[],['--unrooted-allocas']]
