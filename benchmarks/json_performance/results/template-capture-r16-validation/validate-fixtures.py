...
subjects=['lazy-spacer'] if '--lazy-only' in sys.argv else ['test_json_source_length','test_json_template_capture','prior-entry','callback-only']
for stem in subjects:
 source=w/(stem+'.ts');binary=w/(arm+'-'+stem)
 node=subprocess.run(['/opt/homebrew/bin/node','--experimental-strip-types',str(source)],env=clean,capture_output=True,timeout=180);assert node.returncode==0,node.stderr
