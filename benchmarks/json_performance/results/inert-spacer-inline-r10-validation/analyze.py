...
(work / ('full-recheck-analysis.json' if full_recheck else 'full-analysis.json' if with_full else 'options-analysis.json' if options_only else 'recheck-analysis.json' if recheck else 'analysis.json')).write_text(json.dumps({'trials': total, 'phases': outputs}, indent=2) + '\n')
