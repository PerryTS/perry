...
dest = bench / 'results/inert-spacer-inline-r10-validation'
...
manifest = dict(measured_source_commit=json.loads((work / 'provenance.json').read_text())['source_commit'],
                note='R10 rejected: full matrix found five separated slowdowns; longer recheck confirms object parse +0.78%, array stringify +0.80%, numeric stringify +0.70%, all 11 pairs slower. 2572 verified trials in five quiet windows. 292 JSON units, 19 fixtures and 14 options per arm pass. Nine native findings unchanged and unsuppressed; known lazy/fraction gaps preserved. Lint freshness fails. Retained/rotating/short suites prepared but not run; full CI not claimed.',
                files=entries)
