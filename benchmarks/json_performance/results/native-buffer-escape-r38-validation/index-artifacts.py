...
if '--write' in sys.argv:
 ...
 paths=[bench/'NATIVE_BUFFER_ESCAPE_R38.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 assert (timed,verify,calibration)==(1372,305,144)
 assert qualified==dict(timed=952,verify=205,calibration=84)
 m=json.loads((w/'provenance.json').read_text())
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=qualified['timed'],verification_records=qualified['verify'],calibration_trials=qualified['calibration'],archived_trial_counts=dict(timed=timed,verify=verify,calibration=calibration),windows=windows,qualification='Not promoted: small fresh parse +0.63% with separated ranges. Escaped parse improves about6%, large escaped pretty/callback improve about50%; four qualified windows, one excluded initial window retained. No current-source full/changing/retained/access performance window. Frozen R26 reference at0.5.1531; not merged-main evidence.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
