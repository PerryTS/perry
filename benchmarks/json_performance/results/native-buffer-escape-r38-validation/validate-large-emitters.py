...
for ext,data in [('stdout',node.stdout),('stderr',node.stderr)]: (w/('large-emitter-node.'+ext)).write_bytes(data)
...
for roots,setting in [('native','1'),('shadow','0')]:
 binary=w/(arm+'-large-emitter-'+roots)
 ...
 for parser,settings,mode,knobs in cases:
  label=arm+'-large-emitter-'+roots+'-'+parser+'-'+mode
  try:
  ...
  (w/(arm+'-large-emitter-validation.json')).write_text(json.dumps(record,indent=2)+'\n')
  print(label,code,row['matches_node'],protected,moved,flush=True)
assert len(record['rows'])==20
if arm=='candidate':
 assert all(r['exit_code']==0 and r['matches_node'] for r in record['rows']), 'Candidate large-emitter behavior failed; all outcomes preserved.'
