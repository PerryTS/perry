...
source = root / 'test-files/test_json_primitive_tojson_keys.ts'
...
for mode, setting in [('native', '1'), ('shadow', '0')]:
    d = w / (arm + '-primitive-keys-ir-' + mode)
...
(w / (arm + '-primitive-keys-roots.json')).write_text(json.dumps({'rows': rows, 'source_sha256': hashlib.sha256(source.read_bytes()).hexdigest(), 'compiler_sha256': hashlib.sha256((build / 'perry').read_bytes()).hexdigest(), 'runtime_sha256': hashlib.sha256((build / 'libperry_runtime.a').read_bytes()).hexdigest()}, indent=2) + '\n')
