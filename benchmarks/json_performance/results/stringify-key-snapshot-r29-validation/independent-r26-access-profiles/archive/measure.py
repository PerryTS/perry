...
names=['with_lock.py','run_dispatch_focus.py']+[prefix+n for n in ['run-access-profiles.py','profile-cases.json','r26-access-worker','workers-provenance.json','build-provenance.json','source.patch','access-worker.ts','access-worker.js']]+['.work/fixtures/'+f+'.json' for f in ['records_array_1m']]
expected={p:hashlib.sha256((bench/p).read_bytes()).hexdigest() for p in names};token='json-access-r26-profiles-'+str(os.getpid())
...
assert expected[prefix+'r26-access-worker']==next(h for p,h in proof['files'].items() if p.endswith('/candidate-access-worker'))
...
ssh(code)
# Archive locally under ignored WORK while the independent R29 build requires a clean source tree.
