...
for row in json.loads((d/'profiles.json').read_text()):
 c=row['case'];label=c['fixture']+'-'+c['operation']+'-'+c['mode'];assert row['worker_exit']==row['sampler_exit']==0 and row['stop_reason'] is None and row['diagnostic_only'];assert row['worker_sha256']==sha((w/'r26-access-worker').read_bytes())
...
(w/'analysis.json').write_text(json.dumps(dict(source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',diagnostic_only=True,note='Two R26 frozen-worker access sampling diagnostics, with Node executing the complete identical loop count and warmup; checksum and KEEP verified. Access worker does not emit full JSON output. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
