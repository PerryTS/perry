...
original=original_root/'benchmarks/json_performance/.work/stringify-key-snapshot-r29'
...
for n in ['build-provenance.json','build-start.json','build-command-result.json','unit-source.json','unit.log','build.log','validate-and-build.log','lint-results.json','script-lint.log','file-cap.log','worktree-provenance.json']:
 copy(original/n,w/n)
for n in ['build-release.py','validate-and-build.py','run-lint.py']:
 ...
 p=w/n
 if p.exists():p.rename(w/('bootstrap-unused-'+n))
 copy(original/n,p)
