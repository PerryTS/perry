...
lines=['# Root callback keys and reuse scalar string emitters: R29','',(w/'measurement-verdict.md').read_text().strip(),'',
 '## Scope and reference','',
 'The object replacer walk keeps one rewritable key handle across getters, toJSON and replacer callbacks. It reloads key bytes only after callbacks and scopes replacer-pointer use at each call. The same slot is reused for each property. Pretty printing and replacer scalar output now use the existing heap-string provenance and inline-short-string emitters. Escaped strings retain their existing writer. Output allocation, GC policy, parse boundaries, cache admission and thresholds are unchanged.','',
 ...
 'All 81 original reference behavior/options receipts are reused from the exact R26 build. All 81 original candidate executions are fresh. The new emitter regression adds 20 fresh executions per arm: the reference has six scheduled failures, while the candidate must pass all 20. Fresh native/shadow checker runs cover the new fixture. The original 18 checker verdicts are reused only after exact emitted-IR and checker-source equivalence; existing findings remain unsuppressed.','',
 '## Measurement method','',
...
def table(title, phase, retained=False):
    ...
    lines.extend(['### ' + title, '', f"Window: {window['started_utc']} to {window['finished_utc']}.", '',
        '| Fixture / operation | R26 µs | R29 µs | Node µs | Bun µs | CPU change | Ranges |',
        '|---|---:|---:|---:|---:|---:|---|'])
    for row in rows:
        e = row['engines']
    ...
    lines.extend(['', '| Fixture / operation | R26 MiB | R29 MiB | Node MiB | Bun MiB | Peak RSS change MiB |',
        '|---|---:|---:|---:|---:|---:|'])
    for row in rows:
        e = row['engines']
        label = row['fixture'] + ' / ' + row.get('operation', row.get('mode', '')) + (f" / {row['count']} live" if retained else '')
    ...
    if retained:
        lines.extend(['', '| Fixture / operation | R26 after MiB | R29 after MiB | Node after MiB | Bun after MiB | After-RSS change MiB |', '|---|---:|---:|---:|---:|---:|'])
        for row in rows:
            e = row['engines']
...
lines.extend(['## Build fingerprints','','| Artifact | R26 SHA-256 | R29 SHA-256 |','|---|---|---|'])
...
lines.extend(['','The artifact index records archived raw windows, exact sample vectors, validation receipts, source patches and failed attempts. Initial build/lint failures and their corrections are retained explicitly. The candidate compiler and both static archives were built in a separate clean worktree; the copy receipt identifies the exact source and hashes. Prior independent profiles and future parser proposals are not R29 timing results.',''])
assert len(lines)<1900
(bench/'STRINGIFY_EMITTERS_R29.md').write_text('\n'.join(lines))
print('Wrote R29 report with',len(lines),'lines')
