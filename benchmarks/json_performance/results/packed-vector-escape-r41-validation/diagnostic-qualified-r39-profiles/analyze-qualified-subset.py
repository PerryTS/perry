...
selected=[r for r in json.loads((d/'profiles.json').read_text()) if r['case']['operation']=='stringify' or r['case']['fixture']=='records_object_8m']
...
for row in selected:
 ...
 oracle=subprocess.run(cmd,env=clean,capture_output=True,check=True,timeout=120);(d/(label+'.node-oracle.stdout')).write_bytes(oracle.stdout);(d/(label+'.node-oracle.stderr')).write_bytes(oracle.stderr)
...
(w/'qualified-subset-analysis.json').write_text(json.dumps(dict(source_commit='e69d1292110cf9abee24f8690c584652805a4167',reference_source_commit='3aac4d6335da54abeeed73df842decbbe6dd5d71',diagnostic_only=True,note='Four-case follow-up: both wide-object stringify and large-record-object parse profiles retain the500-sample floor. Both wide-object parse profiles remain excluded for insufficient samples. The original six-case analyzer refusal remains preserved; pinned Node executes the complete identical loop and warmup, and checksum plus every VERIFY/LAST/KEEP byte match. CPU/RSS of sampled processes is not benchmark evidence. Inclusive attribution overlaps and cannot be added.',cases=rows),indent=2)+'\n')
