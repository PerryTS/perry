...
if '--write' in sys.argv:
 dirs=[bench/'results'/('quiet-'+w.name+'-'+phase) for phase in ['large-options','rotating','full','full-retry1']]+[bench/'results'/(w.name+'-validation')]
 paths=[bench/'PACKED_VECTOR_ESCAPE_R41.md']+[p for d in dirs for p in sorted(d.rglob('*')) if p.is_file()]
 ...
 assert (timed,verify,calibration)==(3388,630,60)
 assert qualified==dict(timed=1988,verify=380,calibration=60)
 m=json.loads((w/'provenance.json').read_text())
 data=dict(source_commit=m['source_commit'],reference_source_commit=m['main_build']['source_commit'],base_commit=m['base_commit'],timed_trials=qualified['timed'],verification_records=qualified['verify'],calibration_trials=qualified['calibration'],archived_trial_counts=dict(timed=timed,verify=verify,calibration=calibration),windows=windows,qualification='Packed escaping beats both peers on the six large-options rows. Full and fresh-input screens are included; remaining regressions are enumerated in the report. Not promoted or merged-main evidence. No small-options/Korean/changing/retained/access-specific performance run. Earlier R39 profile and scalar-control observations are supplementary historical evidence, not included in these R41 benchmark totals. All 125 canonical candidate output checks pass; known findings remain explicit.',files=[dict(path=str(p.relative_to(root)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in paths])
