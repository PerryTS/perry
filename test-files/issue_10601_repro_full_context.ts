// #10601: the exact repro from the issue -- append a DataView-crafted NaN
// (embedded id 5) as the 7th entry of `test_gap_10479_instanceof_value_kinds.ts`'s
// `badRhs` array, unmodified otherwise. By the time this loop runs, 5 real
// classes have already been declared in file order (K=1, A=2, B=3, Even=4,
// ShortString=5 -- see issue_10601_repro_class_id_confirm.ts), so the
// crafted value's embedded id (5) collides with a REAL, registered class
// (ShortString). Compare against issue_10601_repro_minimal_context.ts, where
// only 1 class is declared and the same crafted bytes correctly throw.
//
// Expected (Node): every `instanceof craftedNaN` line throws TypeError.
// Perry (post-#10592): `sso3 instanceof craftedNaN: true`,
// `newA instanceof craftedNaN: false`, `null instanceof craftedNaN: false`
// -- dispatched through ShortString's [Symbol.hasInstance] instead.
...
__dv.setUint32(0, 0x7ffe0000, false);
__dv.setUint32(4, 5, false);
const craftedNaN: any = __dv.getFloat64(0, false);
