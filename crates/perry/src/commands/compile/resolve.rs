...
//!
//! Native-library manifest parsing (`has_perry_native_library` /
//! `has_perry_native_module` / `parse_native_library_manifest` and the
//! ABI / POD / backend descriptor parsers) was split into the sibling
//! `native_manifest` module to keep this file under the file-size cap;
//! it is re-exported here so callers keep reaching it via `resolve::`.
//! The fns that remain here cover:
//!
...
// Native-library manifest parsing (`perry.nativeLibrary` / `nativeModule`)
// lives in `native_manifest` to keep this file under the file-size cap; the
// symbols are re-exported here so callers keep reaching them via `resolve::`.
mod native_manifest;
pub(super) use native_manifest::{
    has_perry_native_library, has_perry_native_module, parse_native_library_functions,
    parse_native_library_manifest, parse_target_native_config, split_module_spec,
