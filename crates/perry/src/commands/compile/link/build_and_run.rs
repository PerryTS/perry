...
fn winui_bootstrap_import_library(ui_library: &Path) -> Result<PathBuf> {
...
fn deploy_winui_runtime_assets(
    target: Option<&str>,
    needs_ui: bool,
    exe_path: &Path,
) -> Result<()> {
    ...
    if ctx.needs_ui || force_ui {
        ...
        if let Some(ui_lib) = ui_lib_option {
            let winui_import_library = if matches!(target, Some("windows-winui")) {
                Some(winui_bootstrap_import_library(&ui_lib)?)
            } else {
                None
            };
...
"#
    ...
    if !link_cache_status.linked {
        ...
        deploy_winui_runtime_assets(target, ctx.needs_ui, exe_path)?;
