...
#[derive(Debug, Clone)]
pub(super) struct LinkCacheStatus {
    linked: bool,
...
impl LinkCacheStatus {
    ...
    pub(super) fn stats(&self) -> LinkCacheStats {
        LinkCacheStats {
            linked: self.linked,
            skipped: !self.linked,
        }
...
pub(super) fn write_link_cache_manifest(status: &LinkCacheStatus, exe_path: &Path) {
...
fn compute_link_cache_state(
    ...
    let manifest_name = format!(
        "{:016x}.json",
        super::object_cache::djb2_hash(output_identity.as_bytes())
    );
