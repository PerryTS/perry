...
pub(super) fn build_optimized_libs(
    ...
    // stdlib features.
    //
    // The feature list itself is built once, up front, by
    // `auto_optimized_cross_features` (see the early `let cross_features = ...`
    // binding above). This region only consumes the already-computed list.
