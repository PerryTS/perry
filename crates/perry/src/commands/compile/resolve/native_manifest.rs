//! Native-library manifest parsing: the `perry.nativeLibrary` /
//! `perry.nativeModule` package.json config family (#466).
//!
//! Split out of `resolve.rs` (v0.5.10xx) to keep both files under the
//! 2,000-line CI cap. Covers manifest entry detection
//! (`has_perry_native_library` / `has_perry_native_module`), the
//! `parse_native_library_manifest` reader and its ABI / POD / backend /
//! target-config descriptor parsers, the per-arch target-key + prebuilt
//! path resolution helpers, and `validate_abi_version`. The module
//! *resolution* logic (import paths, npm package discovery, `resolve_import`)
//! stays in `resolve.rs`.
...
use super::native_library::native_manifest_target_key;
...
pub(crate) fn parse_native_library_functions(
    package_json: &Path,
...
pub(crate) fn parse_target_native_config(
    package_dir: &Path,
    module_name: &str,
    target_key: &str,
    manifest_path: &str,
...
pub(crate) fn split_module_spec(spec: &str) -> Option<(String, String)> {
...
/// the v0.5.x cycle.
pub(crate) const PERRY_FFI_ABI_VERSION: &str = env!("CARGO_PKG_VERSION");
