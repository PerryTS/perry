...
/// Integrity-restricted receivers of the primed pre-shape's key list. Sabotage:
/// integrity changes keep the ShapeId -> the add lands on a frozen object.
#[test]
fn frozen_sealed_and_non_extensible_receivers_refuse_the_add() {
    let (stdout, hits, memo) = run(
        r#"// A key-add site primed hot on extensible receivers, then fed frozen / sealed /
// non-extensible receivers of the same pre-shape: the add must be refused.
function add(o: any, v: number) { o.z = v; }
let s = 0;
for (let i = 0; i < 2000; i++) { const o: any = { x: i }; add(o, i); s += o.z; }
const r: string[] = [];
for (const [name, f] of [["freeze", Object.freeze], ["seal", Object.seal], ["pe", Object.preventExtensions]] as any) {
  const o: any = f({ x: 1 });
  try { add(o, 5); } catch (e) { /* strict mode throws, sloppy ignores */ }
  r.push(name + ":" + ("z" in o));
}
const again: any = { x: 2 }; add(again, 6);
console.log(s, r.join(" "), again.z, Object.keys(again).join(","));
"#,
    );
    assert_eq!(stdout, r#"1999000 freeze:false seal:false pe:false 6 x,z"#);
    assert!(
        hits + memo > 0,
...
/// `setPrototypeOf(this, Q)` between two adds of one construction. Sabotage:
/// a prototype change keeps the ShapeId -> Q's setter is skipped.
#[test]
...
/// the add hit compares the class id instead of the pre-shape -> wrong slot.
#[test]
...
/// Every add's RHS allocates, so minors and fulls land mid-construction.
/// Sabotage: the site's shapes are not re-noted as carriers after a full
/// trace -> retired ShapeIds are stamped.
#[test]
