...
#[derive(Clone)]
pub(crate) struct SocketAddressState {
    address: IpAddr,
    port: u16,
    flowlabel: u32,
}
...
#[no_mangle]
pub extern "C" fn js_net_block_list_to_json(handle: i64) -> *mut ArrayHeader {
    block_lists()
        .lock()
        .unwrap()
        .get(&handle)
...
pub extern "C" fn js_net_block_list_rules(handle: i64) -> *mut ArrayHeader {
    js_net_block_list_to_json(handle)
}

#[no_mangle]
pub unsafe extern "C" fn js_net_block_list_from_json(handle: i64, value: f64) -> f64 {
    let arr = JsValue::from_bits(value.to_bits()).as_pointer::<ArrayHeader>();
    if arr.is_null() {
        return undefined();
    }
