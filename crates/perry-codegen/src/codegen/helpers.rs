...
/// compile at all. Gated on function count — the defining trait of the
/// pathological minified-bundle case (tens of thousands of functions in one
/// module); ordinary per-file modules stay on the inline diamond and keep the
/// hot fast store.
///
/// `PERRY_FULL_OUTLINE_IC=1`/`on`/`true` forces ON, `=0`/`off`/`false` forces
/// OFF; otherwise auto: `function_count >= PERRY_FULL_OUTLINE_IC_MIN_FUNCS`
/// (default 4000).
pub(crate) fn decide_full_outline_ic(function_count: usize) -> bool {
