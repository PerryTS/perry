...
pub(super) const NODE_CORE_ROWS: &[NativeModSig] = &[
    ...
    // #2135 (Node process parity): process.loadEnvFile(path?) — the runtime
    // accepts the full JS path value so nullish defaults, Buffer, and file URL
    // inputs share the same validation path.
    NativeModSig {
        module: "process",
        has_receiver: false,
        method: "loadEnvFile",
        class_filter: None,
        runtime: "js_process_load_env_file_value",
        args: &[NA_F64],
        ret: NR_VOID,
    },
