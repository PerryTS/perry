...
fn masked_window_fact_for_index(
    ctx: &FnCtx<'_>,
    arr_id: u32,
    index: &Expr,
) -> Option<super::MaskedWindowArrayFact> {
...
/// load is a bare `header + 8 + idx*8` f64 read — no guard call, no hole
/// check, no side exit.
fn lower_masked_window_index_get(
    ctx: &mut FnCtx<'_>,
    arr_id: u32,
    arr_box: &str,
    idx_i32: &str,
    fact: &super::MaskedWindowArrayFact,
) -> String {
    let value = {
        let blk = ctx.block();
...
pub(crate) fn lower_numeric_index_get_for_number_context(
    ...
    if let Expr::LocalGet(arr_id) = object.as_ref() {
        if let Some(fact) = masked_window_fact_for_index(ctx, *arr_id, index.as_ref()) {
            let arr_box = lower_expr(ctx, object)?;
            let idx_i32 = lower_expr_as_i32(ctx, index)?;
            return Ok(Some(lower_masked_window_index_get(
