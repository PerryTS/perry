    ...
    match expr {
        ...
        Expr::LocalSet(id, value) => {
            ...
            if let Some(i32_slot) = ctx.i32_counter_slots.get(id).cloned() {
                ...
                // Within a stable-packed numeric clone, `x | 0` may feed the
                // canonical i32 slot directly: materializing a double here
                // only to convert it back would duplicate the spec ToInt32.
                // Keep both gates explicit. Declared Number types are erased,
                // so a local can still hold a String or BigInt at runtime; and
                // other loop clones own narrower indexed-load contracts that
                // their existing assignment lowering must continue to see.
