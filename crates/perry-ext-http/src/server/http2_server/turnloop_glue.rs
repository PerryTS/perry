...
/// Record which turnloop connection carries a session, both ways.
...
pub(crate) fn queue_turnloop_remote_settings(session_handle: i64) {
    let settings = get_handle::<Http2SessionHandle>(session_handle)
        .map(|s| s.remote_settings.clone())
        .unwrap_or_default();
...
pub(crate) fn bind_turnloop_stream_id(stream_handle: i64, h2_id: i64) {
...
pub(crate) fn bind_turnloop_stream_conn(stream_handle: i64, conn_id: i64) {
    if let Some(stream) = get_handle_mut::<Http2StreamHandle>(stream_handle) {
...
pub(crate) fn queue_turnloop_goaway(session_handle: i64, code: u32, last_stream: u32) {
    if session_handle == 0 {
        return;
    }
