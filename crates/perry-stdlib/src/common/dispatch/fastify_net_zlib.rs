...
/// extern "C" symbols that resolve to perry-ext-net's archive at link time.
/// (perry-stdlib's bundled `net` copy and its `dispatch_net_socket` twin
/// were deleted in tokio lane L4; perry-ext-net is the only provider.)
///
