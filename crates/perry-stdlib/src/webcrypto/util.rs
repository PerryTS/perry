...
pub(super) fn generate_p384_signing_key() -> Option<P384EcdsaSigningKey> {
    ...
    for _ in 0..128 {
        ...
        rng.fill_bytes(&mut bytes);
        if let Ok(key) = P384EcdsaSigningKey::from_slice(&bytes) {
            return Some(key);
        }
    }
    None
...
pub(super) fn generate_p521_signing_key() -> Option<P521EcdsaSigningKey> {
    ...
    for _ in 0..128 {
        ...
        rng.fill_bytes(&mut bytes);
        if let Ok(key) = P521EcdsaSigningKey::from_slice(&bytes) {
            return Some(key);
        }
    }
    None
...
pub(super) fn generate_p384_secret_key() -> Option<P384SecretKey> {
    ...
    for _ in 0..128 {
        let mut bytes = [0u8; 48];
        rng.fill_bytes(&mut bytes);
        if let Ok(key) = P384SecretKey::from_slice(&bytes) {
            return Some(key);
        }
    }
    None
...
pub(super) fn generate_p521_secret_key() -> Option<P521SecretKey> {
    ...
    for _ in 0..128 {
        let mut bytes = [0u8; 66];
        rng.fill_bytes(&mut bytes);
        if let Ok(key) = P521SecretKey::from_slice(&bytes) {
            return Some(key);
        }
    }
    None
