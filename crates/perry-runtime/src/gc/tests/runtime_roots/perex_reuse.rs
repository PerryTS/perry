...
fn perex_reuse_serves_a_whole_global_loop_across_moving_collections() {
    ...
    // Initial setup validates once. Independent receivers now share that
    // immutable binding too, so both loops charge only matching work (#10179).
    let validation = api::WORK - setup.remaining();
    assert!(validation > 0);
    assert_eq!(fresh_work, reused_work);
    let mut repeated_setup = Budget::new(api::WORK);
    let _again = Reuse::new(&scope, &reused, input, &subject, &mut repeated_setup);
    assert_eq!(repeated_setup.remaining(), api::WORK);
}
