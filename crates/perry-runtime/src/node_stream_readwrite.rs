...
pub(super) fn drain_readable_from_events(stream: f64) {
    ...
    let should_emit_end = stream_hidden_ended(stream)
        || has_truthy_hidden(stream, hidden_key(READABLE_AUTO_END_ON_DRAIN_KEY));
    if !stream_destroyed(stream) && should_emit_end {
