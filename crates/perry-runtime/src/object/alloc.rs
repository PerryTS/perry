...
pub unsafe extern "C" fn js_object_assign_one(target_f64: f64, source_f64: f64) -> f64 {
    ...
    // `Object.assign(process.env, parsed)` — how `@next/env` loads `.env`
    // files. `process.env.X` READS lower to `js_getenv` (the real environment),
    // so copying fields into the cached env object leaves every read
    // `undefined`: a Next.js standalone server saw NONE of its `.env` config
    // (myairank's `DATABASE_URL` vanished, mysql2 then connected with an empty
    // user/database and the MySQL handshake timed out). Route each key through
    // the env setter so the write lands where the reads look.
    if crate::process::is_process_env_object(target_f64) {
        let keys = crate::object::js_object_keys_value(source_f64);
        if !keys.is_null() {
            let len = crate::array::js_array_length(keys);
            for i in 0..len {
                let key_val = crate::array::js_array_get_f64(keys, i);
                let key_ptr = crate::value::js_get_string_pointer_unified(key_val)
                    as *const crate::StringHeader;
                if key_ptr.is_null() {
                    continue;
                }
                let value = crate::object::js_object_get_field_by_name_f64(
                    crate::value::js_nanbox_get_pointer(source_f64) as *const ObjectHeader,
                    key_ptr,
                );
                crate::process::js_setenv(key_ptr, value);
            }
        }
        return target_f64;
    }
