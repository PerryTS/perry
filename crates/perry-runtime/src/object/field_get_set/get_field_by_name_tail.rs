    ...
    unsafe {
        ...
        if crate::buffer::is_registered_buffer(obj as usize) {
            if !key.is_null() {
                ...
                // Node's Buffer IS an object (a Uint8Array), so user code can
                // store properties on one — and an own key SHADOWS the
                // prototype method of the same name. Perry keeps buffers
                // outside the object model, so both halves were missing:
                // `buf.foo = v` was dropped and `typeof buf.writeInt8` read
                // `undefined` (methods dispatched on CALL only).
                //
                // mysql2 sizes every outgoing packet with exactly that idiom
                // (`MockBuffer`): it walks `Packet.prototype`, replaces the
                // matching write methods on a ZERO-LENGTH Buffer with a no-op,
                // serializes once to MEASURE, then allocates for real. With the
                // `typeof mock[k] === "function"` probe false, nothing was
                // replaced, the measuring pass wrote into the empty Buffer, and
                // the MySQL handshake died with RangeError [ERR_OUT_OF_RANGE].
