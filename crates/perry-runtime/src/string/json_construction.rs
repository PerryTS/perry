...
/// Allocate a borrowed token after the JSON scanner excluded backslashes,
...
pub(crate) unsafe fn string_from_scanned_json_bytes_known_utf16(
    ...
    // `ParsedStr::Borrowed` reaches this constructor only when the JSON token
    // contained no escape, quote, control or surrogate prefix. The scanner
    // establishes this proof before the allocation, including vector seams.
    init_string_header(header, utf16_len, len, len, 0, STRING_FLAG_JSON_ESCAPE_FREE);
    std::ptr::copy_nonoverlapping(bytes.as_ptr(), data, bytes.len());
    if large_json_leaf {
        crate::json::note_completed_malloc_json_output(len);
