            ...
            if vmaxvq_u8(special) != 0 {
                ...
                for (lane, word) in [
                    ...
                    if hits != 0 {
                        ...
                        // Refine whole vector masks from the first valid ED
                        // prefix onward. Keep this uncommon continuation out
                        // of the ordinary short-token parser frame.
                        return scan_after_valid_ed(&bytes[i..]).map(|n| i + n);
                    }
                }
            }
            i += 16;
...
// Valid Korean ED80..9F must not become a scalar loop for every code point.
// This continuation is entered once, only after the normal scanner found a
// non-surrogate ED prefix. Refine all ED lanes in each vector together.
#[cfg(target_arch = "aarch64")]
#[inline(never)]
