...
pub extern "C" fn js_jsvalue_to_string(value: f64) -> *mut crate::string::StringHeader {
    ...
    if jsval.is_string() {
        ...
        if !ptr.is_null() && crate::value::addr_class::is_above_handle_band(ptr as usize) {
            ...
            // All BufferHeader-backed values must be handled before a GC-header
            // probe. ArrayBuffer, SharedArrayBuffer, and DataView inherit
            // Object.prototype.toString; only byte-indexed buffers decode
            // their contents here.
