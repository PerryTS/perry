...
fn report_stale_address_holders(fault_addr: usize, object_user_ptr: Option<usize>) {
    ...
    while let Some((user, size)) = cursor.next_budgeted(&mut budget) {
        ...
        for i in 0..words {
            ...
            // SAFETY: the cursor only yields user pointers whose header
            // precedes them by GC_HEADER_SIZE.
            let header = unsafe {
                &*((user_addr - crate::gc::GC_HEADER_SIZE) as *const crate::gc::GcHeader)
            };
            out.str("    owner=");
            out.hex(user_addr);
            out.str(" obj_type=");
            out.dec(header.obj_type as u64);
            out.str(" size=");
            out.dec(size as u64);
            out.str(" +");
            out.dec((i * 8) as u64);
    ...
    if found == 0 {
        out.str("    (none — the holder is outside the arena: a runtime side table,\n     an FFI structure, or a register/stack slot)\n");
    }
    out.flush();
}
