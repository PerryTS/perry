...
fn convert(message: Message) -> Incoming {
    match message {
        ...
        Message::Close(frame) => Incoming::Close(
            frame.map(|f| (u16::from(f.code), f.reason.as_str().to_string())),
        ),
...
mod tests {
    ...
    fn text_and_binary_round_trip() {
        ...
        client.send(Message::binary(vec![0u8, 159, 146, 150])).unwrap();
    ...
    fn receive_loop_handles_both_zero_cases() {
        ...
        assert_eq!(
            events,
            vec![Incoming::Text("first".into()), Incoming::Text("second".into())]
        );
    ...
    fn byte_at_a_time_delivery_loses_nothing() {
        let (mut client, mut server) = pair();
        client.send(Message::text("fragmented-by-the-transport")).unwrap();
        let wire = client.take_output();
        let mut seen = Vec::new();
        for byte in &wire {
            seen.extend(server.receive(&[*byte]).unwrap());
        }
        assert_eq!(seen, vec![Incoming::Text("fragmented-by-the-transport".into())]);
    }
    ...
    fn a_ping_is_reported_and_answered_without_the_application_sending() {
        ...
        assert!(!back.is_empty(), "a ping must be answered by the flush inside receive");
        assert_eq!(
