    ...
    if let ast::Expr::Ident(obj_ident) = member.obj.as_ref() {
        ...
        if let Some((module_name, class_name)) = native_instance {
            if let ast::MemberProp::Ident(prop_ident) = &member.prop {
                ...
                } else if module_name == "Headers"
                    && is_headers_instance_method_name(&property_name)
                {
                    // Fetch Headers methods are also first-class method
                    // values. `h.entries` and `h[Symbol.iterator]` must return
                    // the same cached callable, while the call form still
                    // lowers through NativeMethodCall in expr_call.
                    let object_expr = lower_expr(ctx, &member.obj)?;
                    return Ok(Expr::PropertyGet {
                        object: Box::new(object_expr),
                        property: property_name,
...
fn is_headers_instance_method_name(prop: &str) -> bool {
    matches!(
        prop,
        "append"
            | "delete"
