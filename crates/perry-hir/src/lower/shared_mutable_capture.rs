    ...
    fn stmt(&mut self, stmt: &Stmt, scope: u32, loop_depth: u32, top: bool) {
        match stmt {
            Stmt::Let { id, name, init, .. } => {
                ...
                if redeclaration
                    && init.is_some()
                    && (loop_depth > 0 || self.captured.contains(id))
                {
...
fn demote_redeclarations_stmt(stmt: &mut Stmt, ids: &HashSet<LocalId>, seen: &mut HashSet<LocalId>) {
...
fn demote_redeclarations_expr(expr: &mut Expr, ids: &HashSet<LocalId>, seen: &mut HashSet<LocalId>) {
...
fn class_mutates_capture(
    ...
    for_each_nested_capture(c, &HashSet::from([id]), |nested_name, outer_id| {
        classes.get(nested_name).is_some_and(|nested| {
            class_mutates_capture(classes, nested, outer_id, depth + 1)
        })
    })
}
