        ...
        ast::Expr::Ident(ident) => {
            ...
            if let Some(id) = ctx.lookup_local(&name) {
                ...
                if ctx.current_strict {
                    ...
                    // extensions reassign `Date` exactly this way in strict
                    // CJS (`Date = createDate(Date)`), and the old
                    // unconditional throw made the install fail at boot.
                    // Only a genuinely absent binding throws; the helper
                    // does the presence probe + write-back at runtime
                    // (mirrors the `js_global_update` shape for `++x`).
                    // Argument order preserves spec evaluation order: the
                    // RHS evaluates before the reference check throws.
                    return Ok(Expr::Call {
