...
impl SH for Expr {
    fn hash<H: StableHasher>(&self, h: &mut H) {
        match self {
            ...
            Expr::RegisterClassCaptures { class_name, captures } => { tag(h, 12238); class_name.hash(h); for c in captures { c.hash(h); } }
            Expr::ClassCaptureValue { class_name, index } => { tag(h, 12239); class_name.hash(h); index.hash(h); }
