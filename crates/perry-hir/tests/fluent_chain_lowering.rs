...
// commander removal below: it asserted the exact ambient/no-import,
...
// #10704 (the decimal.js/big.js/bignumber.js removal) and #10708 (lru-cache),
// which this PR does not depend on -- all three independently touch this
// file's now-stale test. Confirmed the failure mode is the same for every
// one of the 5 names (including Command): with no import, `new Command(...)`
// now correctly lowers to an unresolved-global reference, matching Node, not
// a regression.
