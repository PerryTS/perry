...
mod computed_and_field_async_tests {
    ...
    fn async_closure_await_in_every_position_is_rewritten() {
        ...
        let cases: Vec<(&str, Vec<Stmt>)> = vec![
            ...
            (
                "throw-await",
                vec![Stmt::Throw(await_(Expr::Integer(1)))],
            ),
            (
                "if-condition",
                vec![Stmt::If {
        ...
        for (i, (label, _body)) in cases.iter().enumerate() {
            ...
            let rewritten = matches!(
                ...
                Some(Expr::Closure { is_async: false, .. })
            );
        ...
        match s {
            ...
            Stmt::If {
                ...
                count_raw_awaits_expr(condition)
                    ...
                    + else_branch.as_ref().map_or(0, |b| count_raw_awaits_stmts(b))
            }
            Stmt::While { condition, body } | Stmt::DoWhile { body, condition } => {
            ...
            Stmt::Try {
                ...
                count_raw_awaits_stmts(body)
                    + catch.as_ref().map_or(0, |c| count_raw_awaits_stmts(&c.body))
    ...
    fn async_generator_linearizes_every_await_position() {
        ...
        let y = |v: Expr| Stmt::Expr(Expr::Yield {
        ...
        for (label, body) in &cases {
            let mut module = Module::new("test");
            module.functions.push(async_gen_with_body(500, body.clone()));
            transform_async_to_generator(&mut module);
            transform_generators(&mut module);
