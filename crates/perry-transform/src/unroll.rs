...
fn compute_loop_escaping_ids(stmts: &[Stmt]) -> std::collections::HashSet<LocalId> {
...
fn collect_escaping_in_stmts(
    ...
    escaping: &mut std::collections::HashSet<LocalId>,
) {
    for s in stmts {
        if let Stmt::For { body, .. } = s {
            let mut inside: HashMap<LocalId, usize> = HashMap::new();
            count_local_refs_stmts(body, &mut inside);
            let mut decls: std::collections::HashSet<LocalId> = std::collections::HashSet::new();
...
fn collect_declared_ids_stmts(stmts: &[Stmt], out: &mut std::collections::HashSet<LocalId>) {
    for s in stmts {
