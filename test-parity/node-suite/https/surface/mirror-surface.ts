import * as https from "node:https";

const KEY = `-----BEGIN PRIVATE KEY-----

7S9Q3thVBhvBcz19oFN2Rmvf
-----END PRIVATE KEY-----
`;

const CERT = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

console.log("https globalAgent export:", Object.keys(https).includes("globalAgent"));
console.log(
  "https globalAgent:",
  typeof https.globalAgent,
  https.globalAgent === https.globalAgent,
  https.globalAgent.defaultPort,
  https.globalAgent.protocol,
  https.globalAgent.keepAlive,
  https.globalAgent.maxFreeSockets,
);

const agent = new https.Agent({ keepAlive: true, maxSockets: 5 });
console.log(
  "https agent props:",
  agent.defaultPort,
  agent.protocol,
  agent.maxSockets,
  agent.keepAlive,
  agent.maxFreeSockets,
);
console.log(
  "https agent methods:",
  typeof agent.createConnection,
  typeof agent.createSocket,
  typeof agent.keepSocketAlive,
  typeof agent.reuseSocket,
  typeof agent.getName,
  typeof agent.destroy,
);
const replacement = () => undefined;
agent.createConnection = replacement;
console.log("https agent override:", agent.createConnection === replacement);

const req = https.request(
  "https://127.0.0.1:1/a?b=1",
  { method: "POST", headers: { "x-test": "1" }, timeout: 1 },
  () => {},
);
console.log(
  "https request overload:",
  req.method,
  req.protocol,
  req.host,
  req.path,
  req.listenerCount("response"),
);
req.on("error", () => {});

const getReq = https.get(
  new URL("https://127.0.0.1:1/get?b=2"),
  { method: "POST", timeout: 1 },
  () => {},
);
console.log(
  "https get overload:",
  getReq.method,
  getReq.protocol,
  getReq.host,
  getReq.path,
  getReq.listenerCount("response"),
);
getReq.on("error", () => {});

const server = https.createServer({ key: KEY, cert: CERT });
console.log(
  "https server methods:",
  typeof server.closeAllConnections,
  typeof server.closeIdleConnections,
  typeof server.setTimeout,
);
server.headersTimeout = 123;
server.keepAliveTimeout = 234;
server.requestTimeout = 345;
server.timeout = 456;
server.maxHeadersCount = 789;
server.maxRequestsPerSocket = 9;
console.log(
  "https server knobs:",
  server.headersTimeout,
  server.keepAliveTimeout,
  server.requestTimeout,
  server.timeout,
  server.maxHeadersCount,
  server.maxRequestsPerSocket,
);
console.log("https server setTimeout:", server.setTimeout(567, () => {}) === server, server.timeout);
server.closeAllConnections();
server.closeIdleConnections();
server.close();
