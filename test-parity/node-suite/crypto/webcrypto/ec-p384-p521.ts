...
for (const curve of curves) {
  ...
  const privateJwk = await crypto.subtle.exportKey("jwk", ecdsa.privateKey) as JsonWebKey;
  ...
  const importedEcdsaJwkPrivate = await crypto.subtle.importKey(
    "jwk",
    privateJwk,
    { name: "ECDSA", namedCurve: curve.name },
    true,
  ...
  const importedJwkSignature = await crypto.subtle.sign(
    { name: "ECDSA", hash: curve.hash },
    importedEcdsaJwkPrivate,
    data,
  );
  console.log(
    `ecdsa ${curve.name}:`,
    ...
    await crypto.subtle.verify({ name: "ECDSA", hash: curve.hash }, ecdsa.publicKey, importedJwkSignature, data),
  );
  ...
  const importedEcdhJwkPrivate = await crypto.subtle.importKey(
    "jwk",
    alicePrivateJwk,
    { name: "ECDH", namedCurve: curve.name },
    true,
  ...
  console.log(
    ...
    Buffer.from(aliceBits).equals(Buffer.from(importedPrivateBits)),
  );
}
