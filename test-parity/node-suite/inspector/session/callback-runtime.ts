...
reportSync("post before connect:", () => session.post("Runtime.evaluate", {}, () => {}));
reportSync("connectToMainThread:", () => session.connectToMainThread());
reportSync("connect:", () => session.connect());

await new Promise<void>((resolve) => {
  session.post("Runtime.evaluate", { expression: "1 + 2", returnByValue: true }, (err, result) => {
    console.log(
      "eval callback:",
      err === null,
      result?.result?.type,
...
await new Promise<void>((resolve) => {
  session.post("Runtime.enable", {}, (err, result) => {
    console.log("enable callback:", err === null, Object.keys(result || {}).length);
    resolve();
  });
});
await new Promise<void>((resolve) => {
  session.post("Runtime.evaluate", { expression: "console.log(\"callback-session-event\")" }, () => {
...
reportSync("post after disconnect:", () => session.post("Runtime.evaluate", {}, () => {}));
