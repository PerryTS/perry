...
2. Without matched static fallback archives, compilation then reports missing
   ...
   `_js_net_socket_once` symbol. A mixed `node:net` plus `node:tls` plaintext
   rejection case exposes another fallback-composition failure where the TLS
   server symbols are absent from the selected archives.
